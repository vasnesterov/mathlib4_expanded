...
theorem integral_hasFDerivWithinAt_of_tendsto_ae (hf : IntervalIntegrable f volume a b)
  ...
  rw [HasFDerivWithinAt]; rw [nhdsWithin_prod_eq]
  have :=
    integral_sub_integral_sub_linear_isLittleO_of_tendsto_ae hf hmeas_a hmeas_b ha hb
    ...
    have I1 : ∀ᶠ u in 𝓝[>] t, (u - t) * y ≤ ∫ w in t..u, (G' w).toReal := by
      ...
      calc
        (u - t) * y = ∫ _ in Icc t u, y := by
          simp only [hu.left.le, MeasureTheory.integral_const, Algebra.id.smul_eq_mul, sub_nonneg, MeasurableSet.univ, Real.volume_Icc, Measure.restrict_apply, univ_inter, ENNReal.toReal_ofReal]
        _ ≤ ∫ w in t..u, (G' w).toReal := by
          ...
          · have C1 : ∀ᵐ x : ℝ ∂volume.restrict (Icc t u), G' x < ∞ :=
            ...
            have : x ∈ Ioo m M := by
              simp only [hm.trans_le hx.left, (hx.right.trans_lt hu.right).trans_le (min_le_left M b), mem_Ioo, and_self_iff]
...
theorem integral_eq_sub_of_hasDerivAt_of_tendsto (hab : a < b) {fa fb}
  ...
  have Fderiv : ∀ x ∈ Ioo a b, HasDerivAt F (f' x) x := by
    ...
    unfold_let F
    rw [update_noteq hy.2.ne]; rw [update_noteq hy.1.ne']
  have hcont : ContinuousOn F (Icc a b) := by
    rw [continuousOn_update_iff]; rw [continuousOn_update_iff]; rw [Icc_diff_right]; rw [Ico_diff_left]
...
theorem intervalIntegrable_deriv_of_nonneg (hcont : ContinuousOn g (uIcc a b))
  ...
  cases' le_total a b with hab hab
  · simp only [uIcc_of_le, min_eq_left, max_eq_right, hab, IntervalIntegrable, hab, Ioc_eq_empty_of_le, integrableOn_empty, and_true_iff] at hcont hderiv hpos ⊢
    exact integrableOn_deriv_of_nonneg hcont hderiv hpos
  · simp only [uIcc_of_ge, min_eq_right, max_eq_left, hab, IntervalIntegrable, Ioc_eq_empty_of_le, integrableOn_empty, true_and_iff] at hcont hderiv hpos ⊢
    exact integrableOn_deriv_of_nonneg hcont hderiv hpos
...
theorem integral_mul_deriv_eq_deriv_mul {u v u' v' : ℝ → A}
  ...
  rw [← integral_deriv_mul_eq_sub hu hv hu' hv']; rw [← integral_sub]
...
theorem integral_comp_smul_deriv''' {f f' : ℝ → ℝ} {g : ℝ → E} (hf : ContinuousOn f [[a, b]])
  ...
  rw [hf.image_uIcc] at hg1; rw [← intervalIntegrable_iff'] at hg1
...
theorem integral_deriv_comp_smul_deriv' {f f' : ℝ → ℝ} {g g' : ℝ → E} (hf : ContinuousOn f [[a, b]])
  ...
  rw [integral_comp_smul_deriv'' hf hff' hf' hg']; rw [integral_eq_sub_of_hasDeriv_right hg hgg' (hg'.mono _).intervalIntegrable]
