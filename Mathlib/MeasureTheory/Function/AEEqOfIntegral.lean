...
theorem ae_const_le_iff_forall_lt_measure_zero {β} [LinearOrder β] [TopologicalSpace β]
  ...
  rw [h_Union]; rw [measure_iUnion_null_iff]
  intro n
  exact hc _ (u_lt n)
    ...
    have B : (∫⁻ x in s, g x ∂μ) ≠ ∞ := by
      ...
      calc
        ...
        _ = N * μ s := by
          simp only [lintegral_const, Set.univ_inter, MeasurableSet.univ, Measure.restrict_apply]
        _ < ∞ := by
          simp only [lt_top_iff_ne_top, s_lt_top.ne, and_false_iff, ENNReal.coe_ne_top, ENNReal.mul_eq_top, Ne.def, not_false_iff, false_and_iff, or_self_iff]
...
theorem ae_nonneg_of_forall_set_integral_nonneg_of_stronglyMeasurable (hfm : StronglyMeasurable f)
  ...
  have h_int_gt : (∫ x in s, f x ∂μ) ≤ b * (μ s).toReal := by
    have h_const_le : (∫ x in s, f x ∂μ) ≤ ∫ _ in s, b ∂μ := by
      ...
      rw [EventuallyLE]; rw [ae_restrict_iff hs]
...
theorem ae_eq_zero_of_forall_set_integral_eq_of_sigmaFinite [SigmaFinite μ] {f : α → E}
  ...
  rw [← @Measure.restrict_univ _ _ μ]; rw [← iUnion_spanningSets μ]; rw [EventuallyEq]; rw [ae_iff]; rw [Measure.restrict_apply' (MeasurableSet.iUnion (measurable_spanningSets μ))]
...
theorem ae_eq_zero_of_forall_set_integral_eq_of_finStronglyMeasurable_trim (hm : m ≤ m0) {f : α → E}
  ...
  · intro s hs hμs
    ...
    rw [← restrict_trim hm μ ht_meas] at hμs; rw [Measure.restrict_apply hs] at hμs; rw [trim_measurableSet_eq hm (hs.inter ht_meas)] at hμs
...
lemma ae_eq_zero_of_forall_set_integral_isCompact_eq_zero'
    ...
    (h'f : ∀ (s : Set β), IsCompact s → ∫ x in s, f x ∂μ = 0) :
    f =ᵐ[μ] 0 := by
  rw [← Measure.restrict_univ (μ := μ)]; rw [← iUnion_compactCovering]
...
theorem AEMeasurable.ae_eq_of_forall_set_lintegral_eq {f g : α → ℝ≥0∞} (hf : AEMeasurable f μ)
  ...
  · congr 1
    rw [lintegral_congr_ae (ofReal_toReal_ae_eq _)]; rw [lintegral_congr_ae (ofReal_toReal_ae_eq _)]
    · exact hfg hs hs'
