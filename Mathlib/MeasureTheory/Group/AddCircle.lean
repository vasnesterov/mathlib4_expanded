...
theorem isAddFundamentalDomain_of_ae_ball (I : Set <| AddCircle T) (u x : AddCircle T)
  ...
  · -- `∀ (g : G), g ≠ 0 → AEDisjoint volume (g +ᵥ I) I`
    ...
    rw [dist_eq_norm]; rw [add_sub_cancel]; rw [div_mul_eq_div_div]; rw [← add_div]; rw [← add_div]; rw [add_self_div_two]; rw [div_le_iff' (by positivity : 0 < (n : ℝ))]; rw [← nsmul_eq_mul]
  ...
  · -- `volume univ ≤ ∑' (g : G), volume (g +ᵥ I)`
    ...
    have hG_card : (Finset.univ : Finset G).card = n := by
      show _ = addOrderOf u
      rw [add_order_eq_card_zmultiples']; rw [Nat.card_eq_fintype_card]; rfl
...
theorem volume_of_add_preimage_eq (s I : Set <| AddCircle T) (u x : AddCircle T)
  ...
  rw [(isAddFundamentalDomain_of_ae_ball I u x hu hI).measure_eq_card_smul_of_vadd_ae_eq_self s hsG]; rw [add_order_eq_card_zmultiples' u]
