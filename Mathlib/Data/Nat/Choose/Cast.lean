...
theorem cast_add_choose {a b : ℕ} : ((a + b).choose a : K) = (a + b)! / (a ! * b !) := by
  rw [cast_choose K (_root_.le_add_right le_rfl)]; rw [add_tsub_cancel_left]
...
theorem cast_choose_two (a : ℕ) : (a.choose 2 : K) = a * (a - 1) / 2 := by
  rw [← cast_descFactorial_two]; rw [descFactorial_eq_factorial_mul_choose]; rw [factorial_two]; rw [mul_comm]; rw [cast_mul]; rw [cast_two]; rw [eq_div_iff_mul_eq (two_ne_zero : (2 : K) ≠ 0)]
