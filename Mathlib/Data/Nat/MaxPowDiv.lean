...
theorem base_mul_eq_succ {p n : ℕ} (hp : 1 < p) (hn : 0 < n) :
  ...
  dsimp [maxPowDiv]
  rw [maxPowDiv.go_eq]; rw [if_pos]; rw [mul_div_right _ this]
  · apply go_succ
  · refine ⟨hp, ?_, by simp⟩
...
theorem base_pow_mul {p n exp : ℕ} (hp : 1 < p) (hn : 0 < n) :
  ...
  | 0 => simp
  | e + 1 =>
    rw [pow_succ]; rw [mul_assoc]; rw [mul_comm]; rw [mul_assoc]; rw [base_mul_eq_succ hp]; rw [mul_comm]; rw [base_pow_mul hp hn]
    · ac_rfl
...
theorem pow_dvd (p n : ℕ) : p ^ (p.maxPowDiv n) ∣ n := by
  ...
  · have : n / p < n := by apply Nat.div_lt_self <;> aesop
    ...
    rw [go_succ]; rw [pow_succ]
    nth_rw 2 [←mod_add_div' n p]
    rw [h.right.right]; rw [zero_add]
