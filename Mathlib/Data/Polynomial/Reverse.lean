...
theorem reflect_eq_zero_iff {N : ℕ} {f : R[X]} : reflect N (f : R[X]) = 0 ↔ f = 0 := by
  rw [ofFinsupp_eq_zero]; rw [reflect]; rw [embDomain_eq_zero]; rw [ofFinsupp_eq_zero]
...
theorem reflect_C_mul_X_pow (N n : ℕ) {c : R} : reflect N (C c * X ^ n) = C c * X ^ revAt N n := by
  ext
  rw [reflect_C_mul]; rw [coeff_C_mul]; rw [coeff_C_mul]; rw [coeff_X_pow]; rw [coeff_reflect]
  split_ifs with h
  ...
  · rw [not_mem_support_iff.mp]
    ...
    apply h
    rw [← mem_support_C_mul_X_pow a]; rw [revAt_invol]
set_option linter.uppercaseLean3 false in
...
theorem reflect_mul_induction (cf cg : ℕ) :
  ...
  · induction' cg with cg hcg
    ...
    · intro N O f g Cf Cg Nf Og
      ...
      rw [← eraseLead_add_C_mul_X_pow g]; rw [mul_add]; rw [reflect_add]; rw [reflect_add]; rw [mul_add]; rw [hcg]; rw [hcg]; all_goals
        try assumption
  ...
  · intro N O f g Cf Cg Nf Og
    ...
    rw [← eraseLead_add_C_mul_X_pow f]; rw [add_mul]; rw [reflect_add]; rw [reflect_add]; rw [add_mul]; rw [hcf]; rw [hcf]; all_goals
      try assumption
    · exact le_add_left card_support_C_mul_X_pow_le_one
...
theorem coeff_zero_reverse (f : R[X]) : coeff (reverse f) 0 = leadingCoeff f := by
  rw [coeff_reverse]; rw [revAt_le (zero_le f.natDegree)]; rw [tsub_zero]; rw [leadingCoeff]
...
theorem reverse_trailingCoeff (f : R[X]) : f.reverse.trailingCoeff = f.leadingCoeff := by
  rw [trailingCoeff]; rw [reverse_natTrailingDegree]; rw [coeff_zero_reverse]
...
@[simp] lemma reverse_mul_X_pow (p : R[X]) (n : ℕ) : reverse (p * X ^ n) = reverse p := by
  ...
  rw [pow_succ']; rw [← mul_assoc]; rw [reverse_mul_X]; rw [ih]
