...
theorem degree_pos_of_root {p : R[X]} (hp : p ≠ 0) (h : IsRoot p a) : 0 < degree p :=
  lt_of_not_ge fun hlt => by
    ...
    rw [IsRoot] at h; rw [this] at h; rw [eval_C] at h
...
theorem natDegree_sum_eq_of_disjoint (f : S → R[X]) (s : Finset S)
  ...
  · push_neg at H
    rw [Finset.sum_eq_zero H]; rw [natDegree_zero]; rw [eq_comm]; rw [show 0 = ⊥ from rfl]; rw [Finset.sup_eq_bot_iff]
    intro x hx
    simp [H x hx]
...
theorem natDegree_comp : natDegree (p.comp q) = natDegree p * natDegree q := by
  ...
  · by_cases p0 : p = 0
    ...
    simp only [coeff_comp_degree_mul_degree q0, p0, mul_eq_zero, leadingCoeff_eq_zero, or_self_iff, ne_zero_of_natDegree_gt (Nat.pos_of_ne_zero q0), pow_ne_zero, Ne.def, not_false_iff]
