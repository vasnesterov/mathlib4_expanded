...
theorem eq_zero_iff_gcd_ne_one {a : ℤ} {p : ℕ} [pp : Fact p.Prime] :
  ...
  rw [Ne]; rw [Int.gcd_comm]; rw [Int.gcd_eq_one_iff_coprime]; rw [(Nat.prime_iff_prime_int.1 pp.1).coprime_iff_not_dvd]; rw [Classical.not_not]; rw [int_cast_zmod_eq_zero_iff_dvd]
