...
theorem Ico_cons_right (h : a ≤ b) : b ::ₘ Ico a b = Icc a b := by
  classical
    rw [Ico]; rw [← Finset.insert_val_of_not_mem right_not_mem_Ico]; rw [Finset.Ico_insert_right h]
    rfl
#align multiset.Ico_cons_right Multiset.Ico_cons_right

theorem Ioo_cons_left (h : a < b) : a ::ₘ Ioo a b = Ico a b := by
  classical
    rw [Ioo]; rw [← Finset.insert_val_of_not_mem left_not_mem_Ioo]; rw [Finset.Ioo_insert_left h]
    rfl
...
theorem Ico_sub_Ico_left (a b c : α) : Ico a b - Ico a c = Ico (max a c) b := by
  rw [Ico]; rw [Ico]; rw [Ico]; rw [← Finset.sdiff_val]; rw [Finset.Ico_diff_Ico_left]
...
theorem Ico_sub_Ico_right (a b c : α) : Ico a b - Ico c b = Ico a (min b c) := by
  rw [Ico]; rw [Ico]; rw [Ico]; rw [← Finset.sdiff_val]; rw [Finset.Ico_diff_Ico_right]
