...
theorem exp_one_near_10 : |exp 1 - 2244083 / 825552| ≤ 1 / 10 ^ 10 := by
  ...
  rw [_root_.abs_one]; rw [abs_of_pos]; all_goals norm_num1
...
theorem exp_neg_one_gt_d9 : 0.36787944116 < exp (-1) := by
  rw [exp_neg]; rw [lt_inv _ (exp_pos _)]
...
theorem exp_neg_one_lt_d9 : exp (-1) < 0.3678794412 := by
  rw [exp_neg]; rw [inv_lt (exp_pos _)]
...
theorem log_two_near_10 : |log 2 - 287209 / 414355| ≤ 1 / 10 ^ 10 := by
  ...
  rw [t] at z
  norm_num1 at z
  rw [one_div (2 : ℝ)] at z; rw [log_inv] at z; rw [← sub_eq_add_neg] at z; rw [_root_.abs_sub_comm] at z
