...
theorem range_orderEmbOfFin (s : Finset α) {k : ℕ} (h : s.card = k) :
  ...
  simp only [orderEmbOfFin, Set.range_comp ((↑) : _ → α) (s.orderIsoOfFin h), RelEmbedding.coe_trans, Set.image_univ, Finset.orderEmbOfFin, RelIso.range_eq, OrderEmbedding.subtype_apply, OrderIso.coe_toOrderEmbedding, eq_self_iff_true, Subtype.range_coe_subtype, Finset.setOf_mem, Finset.coe_inj]
...
theorem orderEmbOfFin_singleton (a : α) (i : Fin 1) :
  ...
  rw [Subsingleton.elim i ⟨0, zero_lt_one⟩]; rw [orderEmbOfFin_zero _ zero_lt_one]; rw [min'_singleton]
...
theorem orderEmbOfFin_unique {s : Finset α} {k : ℕ} (h : s.card = k) {f : Fin k → α}
  ...
  rw [range_orderEmbOfFin]; rw [← Set.image_univ]; rw [← coe_univ]; rw [← coe_image]; rw [coe_inj]
