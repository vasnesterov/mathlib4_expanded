...
theorem embDomain_eq_mapDomain (f : α ↪ β) (v : α →₀ M) : embDomain f v = mapDomain f v := by
  ...
  · rcases h with ⟨a, rfl⟩
    rw [mapDomain_apply f.injective]; rw [embDomain_apply]
...
theorem mapDomain.addMonoidHom_comp_mapRange [AddCommMonoid N] (f : α → β) (g : M →+ N) :
  ...
  ext
  simp only [AddMonoidHom.coe_comp, Finsupp.mapRange_single, Finsupp.mapDomain.addMonoidHom_apply, Finsupp.singleAddHom_apply, eq_self_iff_true, Function.comp_apply, Finsupp.mapDomain_single, Finsupp.mapRange.addMonoidHom_apply]
...
theorem comapDomain_single (f : α → β) (a : α) (m : M)
  ...
  · rw [eq_single_iff, comapDomain_apply, comapDomain_support, ← Finset.coe_subset, coe_preimage,
    ...
    rw [support_single_ne_zero _ hm] at hif; rw [coe_singleton] at hif
...
theorem mapDomain_comapDomain (hf : Function.Injective f) (l : β →₀ M)
  ...
  · rcases Set.mem_range.1 h_cases with ⟨b, hb⟩
    rw [hb.symm]; rw [mapDomain_apply hf]; rw [comapDomain_apply]
...
theorem prod_filter_index [CommMonoid N] (g : α → M → N) :
  ...
  classical
    ...
    rw [support_filter] at hx; rw [Finset.mem_filter] at hx
...
theorem curry_apply (f : α × β →₀ M) (x : α) (y : β) : f.curry x y = f (x, y) := by
  classical
    ...
    rw [Finsupp.curry]; rw [sum_apply]; rw [sum_apply]; rw [sum_eq_single]; rw [this]; rw [if_pos rfl]
    · intro b _ b_ne
      rw [this b]; rw [if_neg b_ne]
    · intro _
      rw [single_zero]; rw [single_zero]; rw [coe_zero]; rw [Pi.zero_apply]; rw [coe_zero]; rw [Pi.zero_apply]
...
def finsuppProdEquiv : (α × β →₀ M) ≃ (α →₀ β →₀ M)
  ...
  left_inv f := by
    rw [Finsupp.uncurry]; rw [sum_curry_index]
    ...
    · intros
      apply single_add
  right_inv f := by
    simp only [Finsupp.curry, Finsupp.uncurry, sum_sum_index, sum_zero_index, sum_add_index, sum_single_index, single_zero, single_add, eq_self_iff_true, forall_true_iff, forall₃_true_iff, (single_sum _ _ _).symm, sum_single]
...
theorem filter_curry (f : α × β →₀ M) (p : α → Prop) :
  ...
  classical
    rw [Finsupp.curry]; rw [Finsupp.curry]; rw [Finsupp.sum]; rw [Finsupp.sum]; rw [filter_sum]; rw [support_filter]; rw [sum_filter]
...
def sumFinsuppAddEquivProdFinsupp {α β : Type*} : (Sum α β →₀ M) ≃+ (α →₀ M) × (β →₀ M) :=
  { sumFinsuppEquivProdFinsupp with
    map_add' := by
      intros
      ext <;>
        simp only [Equiv.toFun_as_coe, Prod.fst_add, Prod.snd_add, add_apply, snd_sumFinsuppEquivProdFinsupp, fst_sumFinsuppEquivProdFinsupp] }
...
def restrictSupportEquiv (s : Set α) (M : Type*) [AddCommMonoid M] :
  ...
  invFun f :=
    ⟨f.embDomain <| Embedding.subtype _, by
      rw [support_embDomain]; rw [Finset.coe_map]; rw [Set.image_subset_iff]
      exact fun x _ => x.2⟩
  left_inv := by
    rintro ⟨f, hf⟩
    ext a
    ...
    rw [embDomain_notin_range]; rw [eq_comm]; rw [←Finsupp.not_mem_support_iff]
...
theorem mem_splitSupport_iff_nonzero (i : ι) : i ∈ splitSupport l ↔ split l i ≠ 0 := by
  rw [splitSupport]; rw [@mem_image _ _ (Classical.decEq _)]; rw [Ne.def]; rw [← support_eq_empty]; rw [← Ne.def]; rw [←
    Finset.nonempty_iff_ne_empty]; rw [split]; rw [comapDomain]; rw [Finset.Nonempty]
...
theorem sigma_support : l.support = l.splitSupport.sigma fun i => (l.split i).support := by
  simp only [Finset.ext_iff, splitSupport, split, comapDomain, @mem_image _ _ (Classical.decEq _), mem_preimage, Sigma.forall, mem_sigma]
  -- porting note: had to add the `Classical.decEq` instance manually
  tauto
