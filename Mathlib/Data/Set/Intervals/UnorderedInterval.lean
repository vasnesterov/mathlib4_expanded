...
lemma _root_.MonotoneOn.mapsTo_uIcc (hf : MonotoneOn f (uIcc a b)) :
  ...
  rw [uIcc]; rw [uIcc]; rw [←hf.map_sup]; rw [←hf.map_inf]; all_goals
...
lemma uIoc_injective_right (a : α) : Injective fun b => Ι b a := by
  ...
  · have hb := (h b).not
    simp only [ha, left_mem_uIoc, not_lt, true_iff_iff, not_mem_uIoc, ← not_le, and_true_iff, not_true, false_and_iff, not_false_iff, true_iff_iff, or_false_iff] at hb
