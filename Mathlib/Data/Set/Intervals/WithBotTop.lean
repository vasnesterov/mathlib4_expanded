...
theorem range_coe : range (some : α → WithTop α) = Iio ⊤ := by
  ext x
  rw [mem_Iio]; rw [lt_top_iff_ne_top]; rw [mem_range]; rw [← none_eq_top]; rw [Option.ne_none_iff_exists]
  rfl
...
theorem image_coe_Iio : (some : α → WithBot α) '' Iio a = Ioo (⊥ : WithBot α) a := by
  rw [← preimage_coe_Iio]; rw [image_preimage_eq_inter_range]; rw [range_coe]; rw [inter_comm]; rw [Ioi_inter_Iio]
#align with_bot.image_coe_Iio WithBot.image_coe_Iio

theorem image_coe_Iic : (some : α → WithBot α) '' Iic a = Ioc (⊥ : WithBot α) a := by
  rw [← preimage_coe_Iic]; rw [image_preimage_eq_inter_range]; rw [range_coe]; rw [inter_comm]; rw [Ioi_inter_Iic]
#align with_bot.image_coe_Iic WithBot.image_coe_Iic

theorem image_coe_Ioi : (some : α → WithBot α) '' Ioi a = Ioi (a : WithBot α) := by
  rw [← preimage_coe_Ioi]; rw [image_preimage_eq_inter_range]; rw [range_coe]; rw [inter_eq_self_of_subset_left (Ioi_subset_Ioi bot_le)]
#align with_bot.image_coe_Ioi WithBot.image_coe_Ioi

theorem image_coe_Ici : (some : α → WithBot α) '' Ici a = Ici (a : WithBot α) := by
  rw [← preimage_coe_Ici]; rw [image_preimage_eq_inter_range]; rw [range_coe]; rw [inter_eq_self_of_subset_left (Ici_subset_Ioi.2 <| bot_lt_coe a)]
...
theorem image_coe_Ioc : (some : α → WithBot α) '' Ioc a b = Ioc (a : WithBot α) b := by
  rw [← preimage_coe_Ioc]; rw [image_preimage_eq_inter_range]; rw [range_coe]; rw [inter_eq_self_of_subset_left (Subset.trans Ioc_subset_Ioi_self <| Ioi_subset_Ioi bot_le)]
#align with_bot.image_coe_Ioc WithBot.image_coe_Ioc

theorem image_coe_Ico : (some : α → WithBot α) '' Ico a b = Ico (a : WithBot α) b := by
  rw [← preimage_coe_Ico]; rw [image_preimage_eq_inter_range]; rw [range_coe]; rw [inter_eq_self_of_subset_left
...
theorem image_coe_Ioo : (some : α → WithBot α) '' Ioo a b = Ioo (a : WithBot α) b := by
  rw [← preimage_coe_Ioo]; rw [image_preimage_eq_inter_range]; rw [range_coe]; rw [inter_eq_self_of_subset_left (Subset.trans Ioo_subset_Ioi_self <| Ioi_subset_Ioi bot_le)]
