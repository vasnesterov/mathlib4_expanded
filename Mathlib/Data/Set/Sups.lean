...
theorem upperClosure_sups [SemilatticeSup α] (s t : Set α) :
  ...
  ext a
  simp only [SetLike.mem_coe, mem_upperClosure, Set.mem_sups, exists_and_left, exists_prop, UpperSet.coe_sup, Set.mem_inter_iff]
  constructor
...
theorem lowerClosure_infs [SemilatticeInf α] (s t : Set α) :
  ...
  ext a
  simp only [SetLike.mem_coe, mem_lowerClosure, Set.mem_infs, exists_and_left, exists_prop, LowerSet.coe_sup, Set.mem_inter_iff]
  constructor
  · rintro ⟨_, ⟨b, hb, c, hc, rfl⟩, ha⟩
