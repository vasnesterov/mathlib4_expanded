...
theorem eq_zero_of_dvd_of_natAbs_lt_natAbs {a b : ℤ} (w : a ∣ b) (h : natAbs b < natAbs a) :
    b = 0 := by
  rw [← natAbs_dvd] at w; rw [← dvd_natAbs] at w; rw [coe_nat_dvd] at w
  rw [← natAbs_eq_zero]
...
theorem ofNat_add_negSucc_of_ge {m n : ℕ} (h : n.succ ≤ m) :
  ...
  rw [negSucc_eq]; rw [ofNat_eq_cast]; rw [ofNat_eq_cast]; rw [← Nat.cast_one]; rw [← Nat.cast_add]; rw [← sub_eq_add_neg]; rw [← Nat.cast_sub h]
