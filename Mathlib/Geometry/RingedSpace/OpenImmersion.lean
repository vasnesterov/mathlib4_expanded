  ...
  PresheafedSpace.isoOfComponents (Iso.refl _) <| by
    ...
    · intro U
      ...
      · induction U using Opposite.rec' with | h U => ?_
        ...
        rfl
    · intro U V i
      simp only [CategoryTheory.eqToIso.hom, TopCat.Presheaf.pushforwardObj_map, Category.assoc, Functor.op_map, Iso.trans_hom, asIso_hom, Functor.mapIso_hom, ← X.presheaf.map_comp]
...
theorem isoRestrict_hom_ofRestrict : H.isoRestrict.hom ≫ Y.ofRestrict _ = f := by
  ...
  · simp only [isoRestrict_hom_c_app, NatTrans.comp_app, eqToHom_refl, ofRestrict_c_app, Category.assoc, whiskerRight_id']
...
instance comp {Z : PresheafedSpace C} (f : X ⟶ Y) [hf : IsOpenImmersion f] (g : Y ⟶ Z)
  ...
  c_iso U := by
    ...
    · have : h.functor.obj U = hg.openFunctor.obj (hf.openFunctor.obj U) := by
        ...
        rw [comp_base]; rw [coe_comp]; rw [show g.base ∘ f.base = fun x => g.base (f.base x) from rfl]; rw [← Set.image_image]
      rw [this]
      infer_instance
    ...
    · have : (Opens.map g.base).obj (h.functor.obj U) = hf.openFunctor.obj U := by
        ...
        -- Porting note : slightly more hand holding here: `g ∘ f` and `fun x => g (f x)`
        rw [coe_comp]; rw [show g.base ∘ f.base = fun x => g.base (f.base x) from rfl]; rw [← Set.image_image g.base f.base]; rw [Set.preimage_image_eq _ hg.base_open.inj]
      rw [this]
      infer_instance
...
theorem inv_invApp (U : Opens X) :
  ...
  rw [← cancel_epi (H.invApp U)]; rw [IsIso.hom_inv_id]
  delta invApp
...
theorem app_invApp (U : Opens Y) :
  ...
  by erw [← Category.assoc]; rw [IsIso.comp_inv_eq]; rw [f.c.naturality]; congr
        ...
        hf.invApp (unop U) ≫
          g.c.app (op (hf.base_open.isOpenMap.functor.obj (unop U))) ≫
            Y.presheaf.map
              (eqToHom
                (by
                  simp only [IsOpenMap.functor, Subtype.mk_eq_mk, unop_op, op_inj_iff, Opens.map, Subtype.coe_mk, Functor.op_obj]
...
theorem pullback_cone_of_left_condition : pullbackConeOfLeftFst f g ≫ f = Y.ofRestrict _ ≫ g := by
  ...
  · induction U using Opposite.rec'
    ...
    simp only [Quiver.Hom.unop_op, TopCat.Presheaf.pushforwardObj_map, app_invApp_assoc, eqToHom_app, eqToHom_unop, Category.assoc, NatTrans.naturality_assoc, Functor.op_map]
...
theorem image_preimage_is_empty (j : Discrete ι) (h : i ≠ j) (U : Opens (F.obj i)) :
  ...
  cases i; cases j
  rw [ι_preservesColimitsIso_hom_assoc] at eq; rw [ι_preservesColimitsIso_hom_assoc] at eq; rw [HasColimit.isoOfNatIso_ι_hom_assoc] at eq; rw [HasColimit.isoOfNatIso_ι_hom_assoc] at eq; rw [TopCat.sigmaIsoSigma_hom_ι] at eq; rw [TopCat.sigmaIsoSigma_hom_ι] at eq
...
def pullbackConeOfLeft : PullbackCone f g := by
  ...
  · use PresheafedSpace.IsOpenImmersion.pullbackConeOfLeftFst f.1 g.1
    ...
    rw [PresheafedSpace.stalkMap.comp] at this; rw [PresheafedSpace.stalkMap.comp] at this
...
def pullbackConeOfLeftIsLimit : IsLimit (pullbackConeOfLeft f g) :=
  PullbackCone.isLimitAux' _ fun s => by
    ...
    · intro x
      ...
      rw [PresheafedSpace.stalkMap.comp] at this; rw [← IsIso.eq_inv_comp] at this
      rw [this]
      infer_instance
    · intro m _ h₂
      rw [← cancel_mono (pullbackConeOfLeft f g).snd]
...
theorem lift_range (H' : Set.range g.1.base ⊆ Set.range f.1.base) :
  ...
  rw [LocallyRingedSpace.comp_val]; rw [SheafedSpace.comp_base]; rw [← this]; rw [← Category.assoc]; rw [coe_comp]
