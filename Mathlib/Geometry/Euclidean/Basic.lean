...
theorem dist_left_midpoint_eq_dist_right_midpoint (p1 p2 : P) :
  ...
  rw [dist_left_midpoint (𝕜 := ℝ) p1 p2]; rw [dist_right_midpoint (𝕜 := ℝ) p1 p2]
...
theorem dist_affineCombination {ι : Type*} {s : Finset ι} {w₁ w₂ : ι → ℝ} (p : ι → P)
  ...
  dsimp only
  rw [dist_eq_norm_vsub V (s.affineCombination ℝ p w₁) (s.affineCombination ℝ p w₂)]; rw [←
    @inner_self_eq_norm_mul_norm ℝ]; rw [Finset.affineCombination_vsub]
...
theorem dist_smul_vadd_eq_dist {v : V} (p₁ p₂ : P) (hv : v ≠ 0) (r : ℝ) :
  ...
  conv_lhs =>
    rw [← mul_self_inj_of_nonneg dist_nonneg dist_nonneg]; rw [dist_smul_vadd_sq]; rw [← sub_eq_zero]; rw [add_sub_assoc]; rw [dist_eq_norm_vsub V p₁ p₂]; rw [← real_inner_self_eq_norm_mul_norm]; rw [sub_self]
  have hvi : ⟪v, v⟫ ≠ 0 := by simpa using hv
  have hd : discrim ⟪v, v⟫ (2 * ⟪v, p₁ -ᵥ p₂⟫) 0 = 2 * ⟪v, p₁ -ᵥ p₂⟫ * (2 * ⟪v, p₁ -ᵥ p₂⟫) := by
    rw [discrim]
    ring
  rw [quadratic_eq_zero_iff hvi hd]; rw [add_left_neg]; rw [zero_div]; rw [neg_mul_eq_neg_mul]; rw [←
...
theorem eq_of_dist_eq_of_dist_eq_of_mem_of_finrank_eq_two {s : AffineSubspace ℝ P}
  ...
  have hv : ∀ v ∈ s.direction, ∃ t₁ t₂ : ℝ, v = t₁ • (c₂ -ᵥ c₁) + t₂ • (p₂ -ᵥ p₁) := by
    ...
    have hr : Set.range b = {c₂ -ᵥ c₁, p₂ -ᵥ p₁} := by
      ...
      simp
    rw [← hbs] at hv; rw [hr] at hv; rw [Submodule.mem_span_insert] at hv
    rcases hv with ⟨t₁, v', hv', hv⟩
    rw [Submodule.mem_span_singleton] at hv'
    rcases hv' with ⟨t₂, rfl⟩
    exact ⟨t₁, t₂, hv⟩
  rcases hv (p -ᵥ p₁) (vsub_mem_direction hps hp₁s) with ⟨t₁, t₂, hpt⟩
  simp only [hpt, inner_add_right, inner_smul_right, ho, mul_zero, add_zero, mul_eq_zero, inner_self_eq_zero, vsub_eq_zero_iff_eq, hc.symm, or_false_iff] at hop
  rw [hop] at hpt; rw [zero_smul] at hpt; rw [zero_add] at hpt; rw [← eq_vadd_iff_vsub_eq] at hpt
  subst hpt
  have hp' : (p₂ -ᵥ p₁ : V) ≠ 0 := by simp [hp.symm]
  have hp₂ : dist ((1 : ℝ) • (p₂ -ᵥ p₁) +ᵥ p₁) c₁ = r₁ := by simp [hp₂c₁]
  rw [← hp₁c₁] at hpc₁ hp₂; rw [dist_smul_vadd_eq_dist _ _ hp'] at hpc₁ hp₂
...
theorem orthogonalProjectionFn_mem {s : AffineSubspace ℝ P} [Nonempty s]
  ...
  rw [← mem_coe]; rw [← Set.singleton_subset_iff]; rw [← inter_eq_singleton_orthogonalProjectionFn]
...
nonrec def orthogonalProjection (s : AffineSubspace ℝ P) [Nonempty s]
  ...
  map_vadd' p v := by
    ...
    have ho :
      ...
      rw [← vsub_right_mem_direction_iff_mem (self_mem_mk' _ _) _]; rw [direction_mk']; rw [vsub_vadd_eq_vsub_sub]; rw [vadd_vsub_assoc]; rw [add_comm]; rw [add_sub_assoc]
      refine' Submodule.add_mem _ (orthogonalProjectionFn_vsub_mem_direction_orthogonal p) _
      rw [Submodule.mem_orthogonal']
      intro w hw
      rw [← neg_sub]; rw [inner_neg_left]; rw [orthogonalProjection_inner_eq_zero _ w hw]; rw [neg_zero]
    have hm :
      ((orthogonalProjection s.direction) v : V) +ᵥ orthogonalProjectionFn s p ∈
...
theorem orthogonalProjection_vsub_orthogonalProjection (s : AffineSubspace ℝ P) [Nonempty s]
  ...
  intro c hc
  rw [← neg_vsub_eq_vsub_rev]; rw [inner_neg_right]; rw [orthogonalProjection_vsub_mem_direction_orthogonal s p c hc]; rw [neg_zero]
...
theorem orthogonalProjection_vadd_eq_self {s : AffineSubspace ℝ P} [Nonempty s]
  ...
  rw [vadd_vsub_assoc] at h; rw [Submodule.add_mem_iff_right _ hv] at h
...
theorem dist_sq_eq_dist_orthogonalProjection_sq_add_dist_orthogonalProjection_sq
  ...
  rw [dist_comm p2 _]; rw [dist_eq_norm_vsub V p1 _]; rw [dist_eq_norm_vsub V p1 _]; rw [dist_eq_norm_vsub V _ p2]; rw [← vsub_add_vsub_cancel p1 (orthogonalProjection s p2) p2]; rw [norm_add_sq_eq_norm_sq_add_norm_sq_iff_real_inner_eq_zero]
...
theorem dist_sq_smul_orthogonal_vadd_smul_orthogonal_vadd {s : AffineSubspace ℝ P} {p1 p2 : P}
  ...
  calc
    dist (r1 • v +ᵥ p1) (r2 • v +ᵥ p2) * dist (r1 • v +ᵥ p1) (r2 • v +ᵥ p2) =
      ...
      rw [dist_eq_norm_vsub V (r1 • v +ᵥ p1)]; rw [vsub_vadd_eq_vsub_sub]; rw [vadd_vsub_assoc]; rw [sub_smul]; rw [add_comm]; rw [add_sub_assoc]
    ...
    _ = dist p1 p2 * dist p1 p2 + (r1 - r2) * (r1 - r2) * (‖v‖ * ‖v‖) := by
      rw [dist_eq_norm_vsub V p1]; rw [abs_mul_abs_self]; rw [mul_assoc]
  ...
  AffineIsometryEquiv.mk'
    ...
    (by
      ...
      have key : a +ᵥ b -ᵥ (v +ᵥ b) +ᵥ (a +ᵥ b) = a + a - v +ᵥ (b -ᵥ b +ᵥ b) := by
        rw [← add_vadd]; rw [vsub_vadd_eq_vsub_sub]; rw [vsub_vadd]; rw [vadd_vsub]
        congr 1
        abel
      dsimp only
...
theorem reflection_eq_self_iff {s : AffineSubspace ℝ P} [Nonempty s]
  ...
  rw [← orthogonalProjection_eq_self_iff]; rw [reflection_apply]
  constructor
  · intro h
    rw [← @vsub_eq_zero_iff_eq V] at h; rw [vadd_vsub_assoc] at h; rw [← two_smul ℝ (↑(orthogonalProjection s p) -ᵥ p)] at h; rw [smul_eq_zero] at h
    norm_num at h
    exact h
  · intro h
    simp [h]
...
theorem reflection_eq_iff_orthogonalProjection_eq (s₁ s₂ : AffineSubspace ℝ P) [Nonempty s₁]
  ...
  constructor
  · intro h
    rw [← @vsub_eq_zero_iff_eq V] at h; rw [vsub_vadd_eq_vsub_sub] at h; rw [vadd_vsub_assoc] at h; rw [add_comm] at h; rw [add_sub_assoc] at h; rw [vsub_sub_vsub_cancel_right] at h; rw [←
      two_smul ℝ ((orthogonalProjection s₁ p : P) -ᵥ orthogonalProjection s₂ p)] at h; rw [smul_eq_zero] at h
    norm_num at h
    exact h
  · intro h
    rw [h]
