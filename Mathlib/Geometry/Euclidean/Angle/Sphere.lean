...
theorem oangle_eq_two_zsmul_oangle_sub_of_norm_eq {x y z : V} (hxyne : x ≠ y) (hxzne : x ≠ z)
  ...
  have hy : y ≠ 0 := by
    rintro rfl
    rw [norm_zero] at hxy; rw [norm_eq_zero] at hxy
    exact hxyne hxy
...
theorem oangle_center_eq_two_zsmul_oangle {s : Sphere P} {p₁ p₂ p₃ : P} (hp₁ : p₁ ∈ s)
  ...
  rw [mem_sphere] at hp₁ hp₂ hp₃; rw [@dist_eq_norm_vsub V] at hp₁ hp₂ hp₃
  rw [oangle]; rw [oangle]; rw [o.oangle_eq_two_zsmul_oangle_sub_of_norm_eq_real _ _ hp₂ hp₁ hp₃]; all_goals
    simp [hp₂p₁, hp₂p₃]
...
theorem two_zsmul_oangle_eq {s : Sphere P} {p₁ p₂ p₃ p₄ : P} (hp₁ : p₁ ∈ s) (hp₂ : p₂ ∈ s)
  ...
  rw [mem_sphere] at hp₁ hp₂ hp₃ hp₄; rw [@dist_eq_norm_vsub V] at hp₁ hp₂ hp₃ hp₄
  rw [oangle]; rw [oangle]; rw [← vsub_sub_vsub_cancel_right p₁ p₂ s.center]; rw [←
      vsub_sub_vsub_cancel_right p₄ p₂ s.center]; rw [o.two_zsmul_oangle_sub_eq_two_zsmul_oangle_sub_of_norm_eq _ _ _ _ hp₂ hp₃ hp₁ hp₄]; all_goals
...
theorem two_zsmul_oangle_center_add_two_zsmul_oangle_eq_pi {s : Sphere P} {p₁ p₂ p₃ : P}
  ...
  rw [← oangle_center_eq_two_zsmul_oangle hp₁ hp₂ hp₃ hp₂p₁ hp₂p₃]; rw [oangle_eq_pi_sub_two_zsmul_oangle_center_right hp₁ hp₃ hp₁p₃]; rw [add_sub_cancel'_right]
...
theorem tan_div_two_smul_rotation_pi_div_two_vadd_midpoint_eq_center {s : Sphere P} {p₁ p₂ : P}
  ...
  rw [map_smul]; rw [smul_smul]; rw [add_comm]; rw [o.tan_oangle_add_right_smul_rotation_pi_div_two]; rw [mul_div_cancel _ (two_ne_zero' ℝ)]
  simpa using h.symm
...
theorem dist_div_cos_oangle_center_div_two_eq_radius {s : Sphere P} {p₁ p₂ : P} (hp₁ : p₁ ∈ s)
  ...
  rw [div_right_comm]; rw [div_eq_mul_inv _ (2 : ℝ)]; rw [mul_comm]; rw [show (2 : ℝ)⁻¹ * dist p₁ p₂ = dist p₁ (midpoint ℝ p₁ p₂) by simp]; rw [← mem_sphere.1 hp₁]; rw [←
    tan_div_two_smul_rotation_pi_div_two_vadd_midpoint_eq_center hp₁ hp₂ h]; rw [←
    oangle_midpoint_rev_left]; rw [oangle]; rw [vadd_vsub_assoc]; rw [show p₂ -ᵥ p₁ = (2 : ℝ) • (midpoint ℝ p₁ p₂ -ᵥ p₁) by simp]; rw [map_smul]; rw [smul_smul]; rw [div_mul_cancel _ (two_ne_zero' ℝ)]; rw [@dist_eq_norm_vsub' V]; rw [@dist_eq_norm_vsub' V]; rw [vadd_vsub_assoc]; rw [add_comm]; rw [o.oangle_add_right_smul_rotation_pi_div_two]; rw [Real.Angle.cos_coe]; rw [Real.cos_arctan]
  norm_cast
  rw [one_div]; rw [div_inv_eq_mul]; rw [←
    mul_self_inj (mul_nonneg (norm_nonneg _) (Real.sqrt_nonneg _)) (norm_nonneg _)]; rw [norm_add_sq_eq_norm_sq_add_norm_sq_real (o.inner_smul_rotation_pi_div_two_right _ _)]; rw [←
    mul_assoc]; rw [mul_comm]; rw [mul_comm _ (Real.sqrt _)]; rw [← mul_assoc]; rw [← mul_assoc]; rw [Real.mul_self_sqrt (add_nonneg zero_le_one (sq_nonneg _))]; rw [norm_smul]; rw [LinearIsometryEquiv.norm_map]
...
theorem dist_div_cos_oangle_center_eq_two_mul_radius {s : Sphere P} {p₁ p₂ : P} (hp₁ : p₁ ∈ s)
    (hp₂ : p₂ ∈ s) (h : p₁ ≠ p₂) :
    dist p₁ p₂ / Real.Angle.cos (∡ p₂ p₁ s.center) = 2 * s.radius := by
  rw [← dist_div_cos_oangle_center_div_two_eq_radius hp₁ hp₂ h]; rw [mul_div_cancel' _ (two_ne_zero' ℝ)]
...
theorem dist_div_sin_oangle_div_two_eq_radius {s : Sphere P} {p₁ p₂ p₃ : P} (hp₁ : p₁ ∈ s)
  ...
  rw [← Real.Angle.abs_cos_eq_abs_sin_of_two_zsmul_add_two_zsmul_eq_pi
    (two_zsmul_oangle_center_add_two_zsmul_oangle_eq_pi hp₁ hp₂ hp₃ hp₁p₂.symm hp₂p₃ hp₁p₃)]; rw [_root_.abs_of_nonneg (Real.Angle.cos_nonneg_iff_abs_toReal_le_pi_div_two.2 _)]
...
theorem dist_div_sin_oangle_eq_two_mul_radius {s : Sphere P} {p₁ p₂ p₃ : P} (hp₁ : p₁ ∈ s)
    (hp₂ : p₂ ∈ s) (hp₃ : p₃ ∈ s) (hp₁p₂ : p₁ ≠ p₂) (hp₁p₃ : p₁ ≠ p₃) (hp₂p₃ : p₂ ≠ p₃) :
    dist p₁ p₃ / |Real.Angle.sin (∡ p₁ p₂ p₃)| = 2 * s.radius := by
  rw [← dist_div_sin_oangle_div_two_eq_radius hp₁ hp₂ hp₃ hp₁p₂ hp₁p₃ hp₂p₃]; rw [mul_div_cancel' _ (two_ne_zero' ℝ)]
...
theorem circumsphere_eq_circumsphere_of_eq_of_eq_of_two_zsmul_oangle_eq {t₁ t₂ : Triangle ℝ P}
  ...
  rw [t₁.circumsphere_eq_of_dist_of_oangle h₁₂ h₁₃ h₂₃]; rw [t₂.circumsphere_eq_of_dist_of_oangle h₁₂ h₁₃ h₂₃]; rw [-- Porting note: was `congrm ⟨((_ : ℝ)⁻¹ / 2) • _ +ᵥ _, _ / _ / 2⟩` and five more lines
    Real.Angle.tan_eq_of_two_zsmul_eq h₂]; rw [Real.Angle.abs_sin_eq_of_two_zsmul_eq h₂]; rw [h₁]; rw [h₃]
