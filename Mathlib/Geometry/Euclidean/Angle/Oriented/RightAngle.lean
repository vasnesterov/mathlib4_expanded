...
theorem oangle_add_left_eq_arccos_of_oangle_eq_pi_div_two {x y : V} (h : o.oangle x y = ↑(π / 2)) :
  ...
  rw [← neg_inj] at h ⊢; rw [oangle_rev] at h ⊢; rw [← oangle_neg_orientation_eq_neg] at h ⊢; rw [neg_inj] at h ⊢
  rw [add_comm]
...
theorem oangle_add_right_smul_rotation_pi_div_two {x : V} (h : x ≠ 0) (r : ℝ) :
  ...
  · have ha : o.oangle x (r • o.rotation (π / 2 : ℝ) x) = -(π / 2 : ℝ) := by
    ...
    rw [← neg_inj] at ha; rw [← oangle_neg_orientation_eq_neg] at ha; rw [@neg_neg Real.Angle] at ha
...
theorem oangle_add_left_smul_rotation_pi_div_two {x : V} (h : x ≠ 0) (r : ℝ) :
  ...
  rw [← neg_inj]; rw [oangle_rev]; rw [← oangle_neg_orientation_eq_neg]; rw [neg_inj]; rw [←
    neg_neg ((π / 2 : ℝ) : Real.Angle)]; rw [← rotation_neg_orientation_eq_neg]; rw [add_comm]
