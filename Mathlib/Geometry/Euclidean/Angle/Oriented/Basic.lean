...
theorem oangle_self (x : V) : o.oangle x x = 0 := by
  rw [oangle]; rw [kahler_apply_self]; norm_cast
...
theorem oangle_add_oangle_rev_neg_left (x y : V) : o.oangle (-x) y + o.oangle (-y) x = 0 := by
  rw [oangle_neg_left_eq_neg_right]; rw [oangle_rev]; rw [add_left_neg]
...
theorem oangle_add_oangle_rev_neg_right (x y : V) : o.oangle x (-y) + o.oangle y (-x) = 0 := by
  rw [o.oangle_rev (-x)]; rw [oangle_neg_left_eq_neg_right]; rw [add_neg_self]
...
theorem oangle_ne_zero_and_ne_pi_iff_linearIndependent {x y : V} :
    o.oangle x y ≠ 0 ∧ o.oangle x y ≠ π ↔ LinearIndependent ℝ ![x, y] := by
  rw [← not_or]; rw [← not_iff_not]; rw [Classical.not_not]; rw [oangle_eq_zero_or_eq_pi_iff_not_linearIndependent]
...
theorem oangle_eq_pi_sub_two_zsmul_oangle_sub_of_norm_eq {x y : V} (hn : x ≠ y) (h : ‖x‖ = ‖y‖) :
  ...
  have hy : y ≠ 0 := by
    rintro rfl
    rw [norm_zero] at h; rw [norm_eq_zero] at h
    exact hn h
...
theorem inner_eq_norm_mul_norm_mul_cos_oangle (x y : V) :
  ...
  have : ‖y‖ ≠ 0 := by simpa using hy
  rw [oangle]; rw [Real.Angle.cos_coe]; rw [Complex.cos_arg]; rw [o.abs_kahler]
...
theorem oangle_eq_of_angle_eq_of_sign_eq {w x y z : V}
  ...
  · have hs' : (o.oangle w x).sign = 0 ∧ (o.oangle y z).sign = 0 := by
    ...
    have hpi : π / 2 ≠ π := by
      intro hpi
      rw [div_eq_iff] at hpi; rw [eq_comm] at hpi; rw [← sub_eq_zero] at hpi; rw [mul_two] at hpi; rw [add_sub_cancel] at hpi
...
theorem oangle_eq_angle_of_sign_eq_one {x y : V} (h : (o.oangle x y).sign = 1) :
  ...
  intro hxy
  rw [hxy] at h; rw [Real.Angle.sign_neg] at h; rw [neg_eq_iff_eq_neg] at h; rw [← SignType.neg_iff] at h; rw [← not_le] at h
...
theorem oangle_eq_neg_angle_of_sign_eq_neg_one {x y : V} (h : (o.oangle x y).sign = -1) :
  ...
  intro hxy
  rw [hxy] at h; rw [← SignType.neg_iff] at h; rw [← not_le] at h
  exact h (Real.Angle.sign_coe_nonneg_of_nonneg_of_le_pi (InnerProductGeometry.angle_nonneg _ _)
    (InnerProductGeometry.angle_le_pi _ _))
...
theorem oangle_smul_add_right_eq_zero_or_eq_pi_iff {x y : V} (r : ℝ) :
  ...
  · rcases h with ⟨m, h, hm⟩
    ...
    rw [smul_add] at h; rw [smul_smul] at h; rw [← add_assoc] at h; rw [← add_smul] at h
    refine' ⟨h, not_and_or.1 fun h0 => _⟩
    obtain ⟨h0, h1⟩ := h0
    rw [h1] at h0 hm
    rw [zero_mul] at h0; rw [add_zero] at h0
    simp [h0] at hm
  · rcases h with ⟨m, h, hm⟩
    ...
    obtain ⟨h0, h1⟩ := h0
    rw [h1] at h0 hm
    rw [zero_mul] at h0; rw [sub_zero] at h0
    simp [h0] at hm
...
theorem abs_oangle_sub_left_toReal_lt_pi_div_two {x y : V} (h : ‖x‖ = ‖y‖) :
  ...
  · rw [oangle_eq_pi_iff_oangle_rev_eq_pi, oangle_eq_pi_iff_sameRay_neg, neg_sub] at hs
    ...
    rw [← add_smul] at hr
    rw [← hr] at h; rw [norm_smul] at h; rw [Real.norm_eq_abs] at h; rw [abs_of_pos (Left.add_pos_of_nonneg_of_pos hr0 one_pos)] at h; rw [mul_left_eq_self₀] at h; rw [or_iff_left (norm_ne_zero_iff.2 hy)] at h; rw [add_left_eq_self] at h
    rw [h] at hr; rw [zero_add] at hr; rw [one_smul] at hr
