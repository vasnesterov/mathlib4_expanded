...
theorem cos_angle_add_of_inner_eq_zero {x y : V} (h : ⟪x, y⟫ = 0) :
  ...
  rw [angle_add_eq_arccos_of_inner_eq_zero h]; rw [Real.cos_arccos (le_trans (by norm_num) (div_nonneg (norm_nonneg _) (norm_nonneg _)))
...
theorem sin_angle_add_of_inner_eq_zero {x y : V} (h : ⟪x, y⟫ = 0) (h0 : x ≠ 0 ∨ y ≠ 0) :
  ...
  rw [angle_add_eq_arcsin_of_inner_eq_zero h h0]; rw [Real.sin_arcsin (le_trans (by norm_num) (div_nonneg (norm_nonneg _) (norm_nonneg _)))
      (div_le_one_of_le _ (norm_nonneg _))]
  rw [mul_self_le_mul_self_iff (norm_nonneg _) (norm_nonneg _)]; rw [norm_add_sq_eq_norm_sq_add_norm_sq_real h]
...
theorem angle_sub_eq_arcsin_of_inner_eq_zero {x y : V} (h : ⟪x, y⟫ = 0) (h0 : x ≠ 0 ∨ y ≠ 0) :
    angle x (x - y) = Real.arcsin (‖y‖ / ‖x - y‖) := by
  rw [← neg_eq_zero] at h; rw [← inner_neg_right] at h
  rw [or_comm] at h0; rw [← neg_ne_zero] at h0; rw [or_comm] at h0
...
theorem angle_eq_arccos_of_angle_eq_pi_div_two {p₁ p₂ p₃ : P} (h : ∠ p₁ p₂ p₃ = π / 2) :
  ...
  rw [angle] at h; rw [← inner_eq_zero_iff_angle_eq_pi_div_two] at h; rw [real_inner_comm] at h; rw [← neg_eq_zero] at h; rw [←
    inner_neg_left] at h; rw [neg_vsub_eq_vsub_rev] at h
