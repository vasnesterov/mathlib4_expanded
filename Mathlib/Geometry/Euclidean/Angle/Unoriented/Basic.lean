...
theorem angle_self {x : V} (hx : x ≠ 0) : angle x x = 0 := by
  unfold angle
  rw [← real_inner_self_eq_norm_mul_norm]; rw [div_self (inner_self_ne_zero.2 hx : ⟪x, x⟫ ≠ 0)]; rw [Real.arccos_one]
...
theorem sin_angle_mul_norm_mul_norm (x y : V) :
  ...
  unfold angle
  rw [Real.sin_arccos]; rw [← Real.sqrt_mul_self (mul_nonneg (norm_nonneg x) (norm_nonneg y))]; rw [← Real.sqrt_mul' _ (mul_self_nonneg _)]; rw [sq]; rw [Real.sqrt_mul_self (mul_nonneg (norm_nonneg x) (norm_nonneg y))]; rw [real_inner_self_eq_norm_mul_norm]; rw [real_inner_self_eq_norm_mul_norm]
  by_cases h : ‖x‖ * ‖y‖ = 0
  · rw [show ‖x‖ * ‖x‖ * (‖y‖ * ‖y‖) = ‖x‖ * ‖y‖ * (‖x‖ * ‖y‖) by ring, h, mul_zero,
      mul_zero, zero_sub]
    cases' eq_zero_or_eq_zero_of_mul_eq_zero h with hx hy
    · rw [norm_eq_zero] at hx
      rw [hx]; rw [inner_zero_left]; rw [zero_mul]; rw [neg_zero]
    · rw [norm_eq_zero] at hy
      rw [hy]; rw [inner_zero_right]; rw [zero_mul]; rw [neg_zero]
  · field_simp [h]
    ring_nf
...
theorem angle_eq_zero_iff {x y : V} : angle x y = 0 ↔ x ≠ 0 ∧ ∃ r : ℝ, 0 < r ∧ y = r • x := by
  rw [angle]; rw [← real_inner_div_norm_mul_norm_eq_one_iff]; rw [Real.arccos_eq_zero]; rw [LE.le.le_iff_eq]; rw [eq_comm]
...
theorem angle_eq_pi_iff {x y : V} : angle x y = π ↔ x ≠ 0 ∧ ∃ r : ℝ, r < 0 ∧ y = r • x := by
  rw [angle]; rw [← real_inner_div_norm_mul_norm_eq_neg_one_iff]; rw [Real.arccos_eq_pi]; rw [LE.le.le_iff_eq]
...
theorem angle_add_angle_eq_pi_of_angle_eq_pi {x y : V} (z : V) (h : angle x y = π) :
  ...
  rw [angle_smul_left_of_neg x z hr]; rw [angle_neg_left]; rw [add_sub_cancel'_right]
...
theorem norm_sub_eq_add_norm_iff_angle_eq_pi {x y : V} (hx : x ≠ 0) (hy : y ≠ 0) :
  ...
  rw [← sq_eq_sq hxy₁ hxy₂] at h; rw [norm_sub_pow_two_real] at h
  calc
...
theorem norm_add_eq_add_norm_iff_angle_eq_zero {x y : V} (hx : x ≠ 0) (hy : y ≠ 0) :
  ...
  obtain ⟨hxy₁, hxy₂⟩ := norm_nonneg (x + y), add_nonneg (norm_nonneg x) (norm_nonneg y)
  rw [← sq_eq_sq hxy₁ hxy₂] at h; rw [norm_add_pow_two_real] at h
  calc
