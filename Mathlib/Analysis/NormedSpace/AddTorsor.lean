...
theorem AffineSubspace.isClosed_direction_iff (s : AffineSubspace 𝕜 Q) :
  ...
  rw [← (IsometryEquiv.vaddConst x).toHomeomorph.symm.isClosed_image]; rw [AffineSubspace.coe_direction_eq_vsub_set_right hx]
  rfl
...
theorem dist_center_homothety (p₁ p₂ : P) (c : 𝕜) :
  ...
  rw [homothety_def]; rw [dist_eq_norm_vsub V]
...
theorem dist_lineMap_left (p₁ p₂ : P) (c : 𝕜) : dist (lineMap p₁ p₂ c) p₁ = ‖c‖ * dist p₁ p₂ := by
  ...
  rw [← dist_zero_right]; rw [← dist_lineMap_lineMap]; rw [lineMap_apply_zero]
...
theorem dist_lineMap_right (p₁ p₂ : P) (c : 𝕜) :
  ...
  rw [← dist_eq_norm']; rw [← dist_lineMap_lineMap]; rw [lineMap_apply_one]
...
theorem eventually_homothety_mem_of_mem_interior (x : Q) {s : Set Q} {y : Q} (hy : y ∈ interior s) :
  ...
  rw [lt_div_iff hxy] at hδ; rw [← norm_smul] at hδ; rw [sub_smul] at hδ; rw [one_smul] at hδ
...
theorem dist_midpoint_midpoint_le (p₁ p₂ p₃ p₄ : V) :
  ...
  rw [Real.norm_eq_abs] at this; rw [abs_two] at this
  exact this
#align dist_midpoint_midpoint_le dist_midpoint_midpoint_le
