...
theorem exists_continuousLinearEquiv_apply_eq [ContinuousSMul R V]
  ...
  { toFun := fun z ↦ z + G z • (y - x)
    ...
    left_inv := fun z ↦ by
      simp only [id_eq, eq_mpr_eq_cast, RingHom.id_apply, smul_eq_mul, AddHom.toFun_eq_coe, AddHom.coe_mk, map_add, map_smulₛₗ, map_sub, Gx, mul_sub, mul_one, add_sub_cancel'_right]
