...
theorem integrable_iff_cauchy_basis [CompleteSpace F] : Integrable I l f vol ↔
  ...
  simp only [exists_prop, Prod.forall, Set.mem_iUnion, exists_imp, prod_mk_mem_set_prod_eq, and_imp, mem_inter_iff, mem_setOf_eq]
  exact
    and_congr Iff.rfl
...
theorem norm_integral_le_of_norm_le {g : ℝⁿ → ℝ} (hle : ∀ x ∈ Box.Icc I, ‖f x‖ ≤ g x)
  ...
  · refine' le_of_tendsto_of_tendsto' hfi.hasIntegral.norm hg.hasIntegral fun π => _
    ...
    simp only [BoxAdditiveMap.toSMul_apply, norm_smul, smul_eq_mul, Real.norm_eq_abs, μ.toBoxAdditive_apply, abs_of_nonneg ENNReal.toReal_nonneg]
...
theorem dist_integralSum_integral_le_of_memBaseSet (h : Integrable I l f vol) (h₀ : 0 < ε)
  ...
  rw [convergenceR] at hπ; rw [dif_pos h₀] at hπ
...
theorem integrable_of_continuousOn [CompleteSpace E] {I : Box ι} {f : ℝⁿ → E}
  ...
  have : ∀ J ∈ π₁.toPrepartition ⊓ π₂.toPrepartition,
    ...
    rw [norm_smul]; rw [Real.norm_eq_abs]; rw [abs_of_nonneg this]; rw [← dist_eq_norm]
  ...
  rw [← Finset.sum_mul]; rw [μ.toBoxAdditive.sum_partition_boxes le_top (h₁p.inf h₂p)]
  exact hε.le
...
theorem HasIntegral.of_bRiemann_eq_false_of_forall_isLittleO (hl : l.bRiemann = false)
  ...
  rw [← g.sum_partition_boxes le_rfl hπp]; rw [mem_closedBall]; rw [integralSum]; rw [← sum_filter_add_sum_filter_not π.boxes fun J => π.tag J ∈ s]; rw [← sum_filter_add_sum_filter_not π.boxes fun J => π.tag J ∈ s]; rw [← add_halves ε]
