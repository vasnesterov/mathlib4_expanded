...
theorem prod_list_swap_mem_alternatingGroup_iff_even_length {l : List (Perm α)}
  ...
  rw [mem_alternatingGroup]; rw [sign_prod_list_swap hl]; rw [← Units.val_eq_one]; rw [Units.val_pow_eq_pow_val]; rw [Units.coe_neg_one]; rw [neg_one_pow_eq_one_iff_even]
  decide
...
theorem finRotate_bit1_mem_alternatingGroup {n : ℕ} :
  ...
  rw [mem_alternatingGroup]; rw [bit1]; rw [sign_finRotate]; rw [pow_bit0']; rw [Int.units_mul_self]; rw [one_pow]
...
theorem two_mul_card_alternatingGroup [Nontrivial α] :
  ...
  rw [← Fintype.card_units_int]; rw [← Fintype.card_congr this]
...
theorem isConj_of {σ τ : alternatingGroup α} (hc : IsConj (σ : Perm α) (τ : Perm α))
  ...
  rw [Subtype.coe_mk] at hπ; rw [Subtype.coe_mk] at hπ
  ...
  · have h2 : 2 ≤ σ.supportᶜ.card := by
    ...
    · simp only [← hπ, coe_mk, Subgroup.coe_mul, Subtype.val]
      ...
      rw [mul_assoc π _ σ]; rw [hd.commute.eq]; rw [coe_inv]; rw [coe_mk]
      simp [mul_assoc]
...
theorem closure_three_cycles_eq_alternating :
  ...
  closure_eq_of_le _ (fun σ hσ => mem_alternatingGroup.2 hσ.sign) fun σ hσ => by
    ...
    rw [List.length_cons] at hn; rw [Nat.succ_inj'] at hn
...
theorem IsThreeCycle.alternating_normalClosure (h5 : 5 ≤ Fintype.card α) {f : Perm α}
  ...
  eq_top_iff.2
    (by
      ...
      rw [← MonoidHom.range_eq_map]; rw [subtype_range]; rw [normalClosure]; rw [MonoidHom.map_closure]
...
theorem nontrivial_of_three_le_card (h3 : 3 ≤ card α) : Nontrivial (alternatingGroup α) := by
  ...
  rw [two_mul_card_alternatingGroup]; rw [card_perm]; rw [← Nat.succ_le_iff]
...
theorem normalClosure_swap_mul_swap_five :
  ...
  rw [eq_top_iff]; rw [← normalClosure_finRotate_five]
  refine' normalClosure_le_normal _
  rw [Set.singleton_subset_iff]; rw [SetLike.mem_coe]; rw [← h5]
...
theorem isConj_swap_mul_swap_of_cycleType_two {g : Perm (Fin 5)} (ha : g ∈ alternatingGroup (Fin 5))
  ...
  rw [← sum_cycleType] at h; rw [h2] at h; rw [Multiset.sum_replicate] at h; rw [smul_eq_mul] at h
  have h : Multiset.card g.cycleType ≤ 3 :=
    le_of_mul_le_mul_right (le_trans h (by simp only [card_fin]; ring_nf)) (by simp)
  rw [mem_alternatingGroup] at ha; rw [sign_of_cycleType] at ha; rw [h2] at ha
  norm_num at ha
  rw [pow_add] at ha; rw [pow_mul] at ha; rw [Int.units_pow_two] at ha; rw [one_mul] at ha; rw [Units.ext_iff] at ha; rw [Units.val_one] at ha; rw [Units.val_pow_eq_pow_val] at ha; rw [Units.coe_neg_one] at ha; rw [neg_one_pow_eq_one_iff_even _] at ha
  swap; · decide
  ...
  · have h04 : (0 : Fin 5) ≠ 4 := by decide
    ...
    rw [Disjoint.cycleType]; rw [(isCycle_swap h04).cycleType]; rw [(isCycle_swap h13).cycleType]; rw [card_support_swap h04]; rw [card_support_swap h13]
    · rfl
...
instance isSimpleGroup_five : IsSimpleGroup (alternatingGroup (Fin 5)) :=
  ⟨fun H => by
    ...
    rw [← SetLike.mem_coe] at gH; rw [← Set.singleton_subset_iff] at gH
    ...
    · obtain ⟨m, hm⟩ := Multiset.exists_cons_of_mem ng
      rw [← sum_cycleType]; rw [hm]; rw [Multiset.sum_cons]
    ...
    · -- The case `n = 4` leads to contradiction, as no element of $A_5$ includes a 4-cycle.
      ...
      contrapose! con
      rw [sign_of_cycleType]; rw [cycleType_of_card_le_mem_cycleType_add_two (by decide) ng]
      dsimp only
      decide
    · -- If `n = 5`, then `g` is itself a 5-cycle, conjugate to `finRotate 5`.
      ...
      rw [cycleType_of_card_le_mem_cycleType_add_two (by decide) ng]; rw [cycleType_finRotate]⟩
