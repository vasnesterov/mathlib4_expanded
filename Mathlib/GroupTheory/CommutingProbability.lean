...
theorem commProb_le_one : commProb M ≤ 1 := by
  ...
  rw [← Nat.cast_pow]; rw [Nat.cast_le]; rw [sq]; rw [← Nat.card_prod]
...
theorem commProb_eq_one_iff [h : Nonempty M] :
  ...
  rw [commProb]; rw [← Set.coe_setOf]; rw [Nat.card_eq_fintype_card]; rw [Nat.card_eq_fintype_card]
...
lemma reciprocalFactors_odd {n : ℕ} (h1 : n ≠ 1) (h2 : Odd n) :
  ...
  · rintro rfl
    norm_num at h2
  rw [reciprocalFactors]; rw [dif_neg h0]; rw [dif_neg h1]; rw [if_neg (Nat.odd_iff_not_even.mp h2)]
...
theorem commProb_reciprocal (n : ℕ) :
  ...
  · have := div_two_lt h0
    rw [reciprocalFactors_even h0 h2]; rw [commProb_cons]; rw [commProb_reciprocal (n / 2)]; rw [commProb_odd (by norm_num)]
    field_simp [h0, h2.two_dvd]
    norm_num
  · have := div_four_lt h0 h1
    rw [reciprocalFactors_odd h1 h2]; rw [commProb_cons]; rw [commProb_reciprocal (n / 4 + 1)]
    have key : n % 4 = 1 ∨ n % 4 = 3 := Nat.odd_mod_four_iff.mp (Nat.odd_iff.mp h2)
    have hn : Odd (n % 4) := by rcases key with h | h <;> rw [h] <;> norm_num
    rw [commProb_odd (hn.mul h2)]; rw [div_mul_div_comm]; rw [mul_one]; rw [div_eq_div_iff]; rw [one_mul]; all_goals norm_cast
