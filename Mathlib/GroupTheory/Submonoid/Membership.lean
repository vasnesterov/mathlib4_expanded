...
abbrev groupPowers {x : M} {n : ℕ} (hpos : 0 < n) (hx : x ^ n = 1) : Group (powers x) where
  ...
  zpow_neg' m x := Subtype.ext <| by
    ...
    rw [Int.negSucc_coe]; rw [← Int.add_mul_emod_self (b := (m + 1 : ℕ))]
    nth_rw 1 [← mul_one ((m + 1 : ℕ) : ℤ)]
    rw [← sub_eq_neg_add]; rw [← mul_sub]; rw [← Int.coe_pred_of_pos hpos]; norm_cast
...
theorem sup_eq_range (s t : Submonoid N) : s ⊔ t = mrange (s.subtype.coprod t.subtype) := by
  rw [mrange_eq_map]; rw [← mrange_inl_sup_mrange_inr]; rw [map_sup]; rw [map_mrange]; rw [coprod_comp_inl]; rw [map_mrange]; rw [coprod_comp_inr]; rw [range_subtype]; rw [range_subtype]
