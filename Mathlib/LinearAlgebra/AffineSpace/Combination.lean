...
theorem attach_affineCombination_of_injective [DecidableEq P] (s : Finset P) (w : P → k) (f : s → P)
  ...
  simp only [affineCombination, weightedVSubOfPoint_apply, id.def, vadd_right_cancel_iff, Function.comp_apply, AffineMap.coe_mk]
...
theorem map_affineCombination {V₂ P₂ : Type*} [AddCommGroup V₂] [Module k V₂] [AffineSpace V₂ P₂]
  ...
  simp only [weightedVSubOfPoint_apply, RingHom.id_apply, AffineMap.map_vadd, LinearMap.map_smulₛₗ, AffineMap.linearMap_vsub, map_sum, Function.comp_apply]
...
theorem weightedVSub_weightedVSubVSubWeights [DecidableEq ι] (p : ι → P) {i j : ι} (hi : i ∈ s)
  ...
  rw [weightedVSubVSubWeights]; rw [← affineCombination_vsub]; rw [s.affineCombination_affineCombinationSingleWeights k p hi]; rw [s.affineCombination_affineCombinationSingleWeights k p hj]
...
theorem affineCombination_affineCombinationLineMapWeights [DecidableEq ι] (p : ι → P) {i j : ι}
  ...
  rw [affineCombinationLineMapWeights]; rw [← weightedVSub_vadd_affineCombination]; rw [weightedVSub_const_smul]; rw [s.affineCombination_affineCombinationSingleWeights k p hi]; rw [s.weightedVSub_weightedVSubVSubWeights k p hj hi]; rw [AffineMap.lineMap_apply]
...
theorem weightedVSub_mem_vectorSpan {s : Finset ι} {w : ι → k} (h : ∑ i in s, w i = 0)
  ...
  classical
    ...
    · rw [vectorSpan_range_eq_span_range_vsub_right k p i0, ← Set.image_univ,
      ...
      rw [Finsupp.total_apply]; rw [Finsupp.onFinset_sum hwx]
...
theorem affineCombination_mem_affineSpan [Nontrivial k] {s : Finset ι} {w : ι → k}
  ...
  classical
    ...
    have hw1 : ∑ i in s, w1 i = 1 := by
      simp only [Pi.const_zero, Finset.sum_update_of_mem hi1, Pi.zero_apply, Finset.sum_const_zero, add_zero]
    ...
    · rcases isEmpty_or_nonempty ι with (hι | ⟨⟨i0⟩⟩)
      ...
      · rw [vectorSpan_range_eq_span_range_vsub_right k p i0, ← Set.image_univ,
        ...
        have hw : ∑ i in insert i0 l.support, w i = 0 := by
          ...
          simp only [Finsupp.mem_support_iff, ne_eq, Finset.mem_insert, true_or, not_true, Function.const_apply, Finset.sum_const_zero, add_zero, sub_self]
        use hw
        have hz : w i0 • (p i0 -ᵥ p i0 : V) = 0 := (vsub_self (p i0)).symm ▸ smul_zero _
        change (fun i => w i • (p i -ᵥ p i0 : V)) i0 = 0 at hz
        rw [Finset.weightedVSub_eq_weightedVSubOfPoint_of_sum_eq_zero _ w p hw (p i0)]; rw [Finset.weightedVSubOfPoint_apply]; rw [← hv]; rw [Finsupp.total_apply]; rw [@Finset.sum_insert_zero _ _ l.support i0 _ _ _ hz]
...
theorem eq_affineCombination_of_mem_affineSpan {p1 : P} {p : ι → P}
  ...
  classical
    ...
    rw [affineSpan_nonempty] at hn; rw [Set.range_nonempty_iff_nonempty] at hn
    cases' hn with i0
    ...
    rw [direction_affineSpan] at hd; rw [mem_vectorSpan_iff_eq_weightedVSub] at hd
    rcases hd with ⟨s, w, h, hs⟩
    let s' := insert i0 s
    let w' := Set.indicator (↑s) w
    have h' : ∑ i in s', w' i = 0 := by
      rw [← h]; rw [Set.sum_indicator_subset _ (Finset.subset_insert i0 s)]
    ...
    have hw0 : ∑ i in s', w0 i = 1 := by
      ...
      simp only [Finset.mem_insert, true_or, not_true, Function.const_apply, Finset.sum_const_zero, add_zero]
