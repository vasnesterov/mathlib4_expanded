...
theorem mem_iInf' {ι} {s : ι → Filter α} {U : Set α} :
  ...
  · exact dif_neg hi
  · simp only [iInter_dite, biInter_eq_iInter, dif_pos (Subtype.coe_prop _), Subtype.coe_eta, iInter_univ, inter_univ, eq_self_iff_true, true_and_iff]
#align filter.mem_infi' Filter.mem_iInf'
...
theorem iInf_sets_eq {f : ι → Filter α} (h : Directed (· ≥ ·) f) [ne : Nonempty ι] :
  ...
  (congr_arg Filter.sets this.symm).trans <| by simp
...
theorem iInf_sets_eq_finite' (f : ι → Filter α) :
  ...
  rw [← iInf_sets_eq_finite]; rw [← Equiv.plift.surjective.iInf_comp]
  rfl
#align filter.infi_sets_eq_finite' Filter.iInf_sets_eq_finite'
...
instance : Coframe (Filter α) :=
  { Filter.instCompleteLatticeFilter with
    iInf_sup_le_sup_sInf := fun f s t ⟨h₁, h₂⟩ => by
      rw [iInf_subtype']
      rw [sInf_eq_iInf'] at h₂; rw [iInf_sets_eq_finite] at h₂; rw [mem_iUnion] at h₂
      obtain ⟨u, hu⟩ := h₂
      ...
      rintro ⟨i⟩ u _ ih
      rw [Finset.inf_insert]; rw [sup_inf_left]
...
theorem iInf_sets_induct {f : ι → Filter α} {s : Set α} (hs : s ∈ iInf f) {p : Set α → Prop}
  ...
  case insert ih =>
    rw [Finset.inf_insert] at his; rw [mem_inf_iff] at his
...
theorem mem_inf_principal' {f : Filter α} {s t : Set α} : s ∈ f ⊓ 𝓟 t ↔ tᶜ ∪ s ∈ f := by
  simp only [← le_principal_iff, (isCompl_principal s).le_left_iff, disjoint_assoc, inf_principal, ← (isCompl_principal (t ∩ sᶜ)).le_right_iff, compl_inter, compl_compl]
...
theorem mem_traverse_iff (fs : List β') (t : Set (List α')) :
  ...
  · induction fs generalizing t
    case nil =>
      simp only [sequence, mem_pure, imp_self, forall₂_nil_left_iff, exists_eq_left, Set.pure_def, singleton_subset_iff, traverse_nil]
    case cons b fs ih =>
      intro ht
      rcases mem_seq_iff.1 ht with ⟨u, hu, v, hv, ht⟩
