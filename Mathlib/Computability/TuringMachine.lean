...
theorem BlankExtends.above_of_le {Γ} [Inhabited Γ] {l l₁ l₂ : List Γ} :
  ...
  rw [List.append_assoc]; rw [← List.replicate_add]; rw [tsub_add_cancel_of_le]
...
def ListBlank.nth {Γ} [Inhabited Γ] (l : ListBlank Γ) (n : ℕ) : Γ := by
  ...
  · rw [List.getI_eq_default _ h₂]
  rw [List.getI_eq_get _ h₂]; rw [List.get_append_right' h]; rw [List.get_replicate]
...
theorem ListBlank.ext {Γ} [i : Inhabited Γ] {L₁ L₂ : ListBlank Γ} :
  ...
  · simp only [List.get_append _ h', List.get?_eq_get h, List.get?_eq_get h', ← List.getI_eq_get _ h, ← List.getI_eq_get _ h', H]
  · simp only [List.get_append_right' h', List.get_replicate, List.get?_eq_get h, List.get?_len_le h', ← List.getI_eq_default _ h', H, List.getI_eq_get _ h]
...
theorem ListBlank.nth_modifyNth {Γ} [Inhabited Γ] (f : Γ → Γ) (n i) (L : ListBlank Γ) :
  ...
  · cases i <;> simp only [ListBlank.nth_zero, if_true, ListBlank.head_cons, ListBlank.modifyNth, ListBlank.nth_succ, if_false, ListBlank.tail_cons, Nat.zero_eq]
  · cases i
...
def ListBlank.bind {Γ Γ'} [Inhabited Γ] [Inhabited Γ'] (l : ListBlank Γ) (f : Γ → List Γ')
  ...
  rw [List.append_bind]; rw [mul_comm]; congr
...
theorem Tape.right₀_nth {Γ} [Inhabited Γ] (T : Tape Γ) (n : ℕ) : T.right₀.nth n = T.nth n := by
  cases n <;> simp only [Tape.nth, Tape.right₀, Int.ofNat_zero, ListBlank.nth_zero, ListBlank.nth_succ, ListBlank.head_cons, ListBlank.tail_cons, Nat.zero_eq]
...
theorem Tape.move_right_nth {Γ} [Inhabited Γ] (T : Tape Γ) (i : ℤ) :
  ...
  rw [Tape.move_left_nth]; rw [add_sub_cancel]
...
theorem Tape.write_move_right_n {Γ} [Inhabited Γ] (f : Γ → Γ) (L R : ListBlank Γ) (n : ℕ) :
  ...
  · simp only [ListBlank.nth_zero, ListBlank.modifyNth, iterate_zero_apply, Nat.zero_eq]
    rw [← Tape.write_mk']; rw [ListBlank.cons_head_tail]
  simp only [ListBlank.head_cons, ListBlank.nth_succ, ListBlank.modifyNth, Tape.move_right_mk', ListBlank.tail_cons, iterate_succ_apply, IH]
...
theorem Tape.map_move {Γ Γ'} [Inhabited Γ] [Inhabited Γ'] (f : PointedMap Γ Γ') (T : Tape Γ) (d) :
  ...
  cases T
  cases d <;> simp only [Tape.move, Tape.map, ListBlank.head_map, eq_self_iff_true, ListBlank.map_cons, and_self_iff, ListBlank.tail_map]
...
theorem stmts_trans {M : Λ → Stmt₁} {S : Finset Λ} {q₁ q₂ : Stmt₁} (h₁ : q₁ ∈ stmts₁ q₂) :
  ...
  simp only [stmts, Finset.mem_insertNone, Finset.mem_biUnion, Option.mem_def, Option.some.injEq, forall_eq', exists_imp, and_imp]
...
theorem trTape'_move_right (L R : ListBlank Γ) :
  ...
  · rfl
  rw [iterate_succ_apply]; rw [iterate_succ_apply']; rw [Tape.move_left_right]; rw [IH]
...
theorem tr_respects {enc₀} :
  ...
  fun_respects.2 fun ⟨l₁, v, T⟩ ↦ by
    ...
    case move d q IH =>
      cases d <;>
          simp only [trNormal, iterate, stepAux_move, stepAux, ListBlank.head_cons, Tape.move_left_mk', ListBlank.cons_head_tail, ListBlank.tail_cons, trTape'_move_left enc0, trTape'_move_right enc0] <;>
        apply IH
    case write f q IH =>
      simp only [trNormal, stepAux_read dec enc0 encdec, stepAux]
      refine' ReflTransGen.head rfl _
      obtain ⟨a, R, rfl⟩ := R.exists_cons
      rw [tr]; rw [Tape.mk'_head]; rw [stepAux_write]; rw [ListBlank.head_cons]; rw [stepAux_move]; rw [trTape'_move_left enc0]; rw [ListBlank.head_cons]; rw [ListBlank.tail_cons]; rw [Tape.write_mk']
      apply IH
    case load a q IH =>
...
theorem stk_nth_val {K : Type*} {Γ : K → Type*} {L : ListBlank (∀ k, Option (Γ k))} {k S} (n)
  ...
  rw [← proj_map_nth]; rw [hL]; rw [← List.map_reverse]; rw [ListBlank.nth_mk]; rw [List.getI_eq_iget_get?]; rw [List.get?_map]
...
theorem addBottom_nth_snd (L : ListBlank (∀ k, Option (Γ k))) (n : ℕ) :
  ...
  conv => rhs; rw [← addBottom_map L]; rw [ListBlank.nth_map]
...
theorem tr_respects_aux₂ {k : K} {q : Stmt₂₁} {v : σ} {S : ∀ k, List (Γ k)}
  ...
  case push f =>
    ...
    · subst k'
      ...
      · rw [List.getI_eq_get, List.get_append_right'] <;>
          simp only [h, List.get_singleton, List.length_map, List.length_reverse, Nat.succ_pos', List.length_append, lt_add_iff_pos_right, List.length, le_refl]
      rw [← proj_map_nth]; rw [hL]; rw [ListBlank.nth_mk]
      cases' lt_or_gt_of_ne h with h h
      · rw [List.getI_append]
        simpa only [List.length_map, List.length_reverse] using h
      · rw [gt_iff_lt] at h
        rw [List.getI_eq_default]; rw [List.getI_eq_default]; all_goals
  ...
  case peek f =>
    ...
    cases e : S k; · rfl
    rw [List.length_cons]; rw [iterate_succ']; rw [Function.comp]; rw [Tape.move_right_left]; rw [Tape.move_right_n_head]; rw [Tape.mk'_nth_nat]; rw [addBottom_nth_snd]; rw [stk_nth_val _ (hL k)]; rw [e]; rw [List.reverse_cons]; rw [← List.length_reverse]; rw [List.get?_concat_length]
    rfl
  case pop f =>
    cases' e : S k with hd tl
    · simp only [Tape.mk'_head, ListBlank.head_cons, Tape.move_left_mk', List.length, Tape.write_mk', List.head?, iterate_zero_apply, List.tail_nil]
        ...
        ⟨_, fun k' ↦ _, by
          erw [List.length_cons, Tape.move_right_n_head, Tape.mk'_nth_nat, addBottom_nth_succ_fst,
            ...
            show (List.cons hd tl).reverse.get? tl.length = some hd by
              rw [List.reverse_cons]; rw [← List.length_reverse]; rw [List.get?_concat_length],
            List.head?, List.tail]⟩
      refine' ListBlank.ext fun i ↦ _
      rw [ListBlank.nth_map]; rw [ListBlank.nth_modifyNth]; rw [proj]; rw [PointedMap.mk_val]
      by_cases h' : k' = k
...
theorem tr_respects_aux₁ {k} (o q v) {S : List (Γ k)} {L : ListBlank (∀ k, Option (Γ k))}
  ...
  rw [stk_nth_val _ hL]; rw [List.get?_eq_get]; rfl; rwa [List.length_reverse]
#align turing.TM2to1.tr_respects_aux₁ Turing.TM2to1.tr_respects_aux₁
...
theorem tr_respects_aux {q v T k} {S : ∀ k, List (Γ k)}
  ...
  have := hgo.tail' rfl
  rw [tr] at this; rw [TM1.stepAux] at this; rw [Tape.move_right_n_head] at this; rw [Tape.mk'_nth_nat] at this; rw [addBottom_nth_snd] at this; rw [stk_nth_val _ (hT k)] at this; rw [List.get?_len_le (le_of_eq (List.length_reverse _))] at this; rw [Option.isNone] at this; rw [cond] at this; rw [hrun] at this; rw [TM1.stepAux] at this
...
theorem tr_respects : Respects (TM2.step M) (TM1.step (tr M)) TrCfg := by
  -- Porting note: `simp`s are required for beta reductions.
  intro c₁ c₂ h
...
theorem trCfg_init (k) (L : List (Γ k)) : TrCfg (TM2.init k L) (TM1.init (trInit k L) : Cfg₂₁) := by
  ...
  · refine' ⟨ListBlank.mk (L.reverse.map fun a ↦ update default k (some a)), fun k' ↦ _⟩
    refine' ListBlank.ext fun i ↦ _
    rw [ListBlank.map_mk]; rw [ListBlank.nth_mk]; rw [List.getI_eq_iget_get?]; rw [List.map_map]
    ...
    rw [this]; rw [List.get?_map]; rw [proj]; rw [PointedMap.mk_val]
    simp only []
    by_cases h : k' = k
    · subst k'
      simp only [Function.update_same]
      rw [ListBlank.nth_mk]; rw [List.getI_eq_iget_get?]; rw [← List.map_reverse]; rw [List.get?_map]
    · simp only [Function.update_noteq h]
      rw [ListBlank.nth_mk]; rw [List.getI_eq_iget_get?]; rw [List.map]; rw [List.reverse_nil]
