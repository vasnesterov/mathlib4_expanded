...
def quotientKerEquivOfRightInverse {g : S → R} (hf : Function.RightInverse g f) :
  ...
  { kerLift f with
    ...
    left_inv := by
      ...
      simp only [Submodule.Quotient.quot_mk_eq_mk, Ideal.Quotient.mk_eq_mk, kerLift_mk, Function.comp_apply, hf (f x)]
    right_inv := hf }
...
theorem mk_ker {I : Ideal R} : RingHom.ker (Quotient.mk I) = I := by
  ext
  rw [RingHom.ker]; rw [mem_comap]; rw [@Submodule.mem_bot _ _ _ _ Semiring.toModule _]; rw [Quotient.eq_zero_iff_mem]
...
theorem ker_quotient_lift {S : Type v} [CommRing S] {I : Ideal R} (f : R →+* S)
  ...
  · intro hx
    ...
    rw [RingHom.mem_ker] at hx; rw [← hy] at hx; rw [Ideal.Quotient.lift_mk] at hx; rw [← RingHom.mem_ker] at hx
  ...
  · intro hx
    ...
    obtain ⟨y, hy⟩ := hx
    rw [RingHom.mem_ker]; rw [← hy.right]; rw [Ideal.Quotient.lift_mk]
    exact hy.left
...
theorem mem_quotient_iff_mem_sup {I J : Ideal R} {x : R} :
  ...
  rw [← mem_comap]; rw [comap_map_of_surjective (Quotient.mk I) Quotient.mk_surjective]; rw [←
    RingHom.ker_eq_comap_bot]; rw [mk_ker]
...
def Quotient.liftₐ (I : Ideal A) (f : A →ₐ[R₁] B) (hI : ∀ a : A, a ∈ I → f a = 0) :
  ...
  {-- this is IsScalarTower.algebraMap_apply R₁ A (A ⧸ I) but the file `Algebra.Algebra.Tower`
    ...
    commutes' := fun r => by
      ...
      rw [this]; rw [Ideal.Quotient.algebraMap_eq]; rw [RingHom.toFun_eq_coe]; rw [Ideal.Quotient.lift_mk]; rw [AlgHom.coe_toRingHom]; rw [Algebra.algebraMap_eq_smul_one]; rw [Algebra.algebraMap_eq_smul_one]; rw [map_smul]; rw [map_one] }
...
def quotientEquiv (I : Ideal R) (J : Ideal S) (f : R ≃+* S) (hIJ : J = I.map (f : R →+* S)) :
    R ⧸ I ≃+* S ⧸ J :=
  {
    ...
    left_inv := by
      rintro ⟨r⟩
      simp only [Submodule.Quotient.quot_mk_eq_mk, Quotient.mk_eq_mk, RingHom.toFun_eq_coe, quotientMap_mk, RingEquiv.coe_toRingHom, RingEquiv.symm_apply_apply]
    right_inv := by
      rintro ⟨s⟩
      simp only [Submodule.Quotient.quot_mk_eq_mk, Quotient.mk_eq_mk, RingHom.toFun_eq_coe, quotientMap_mk, RingEquiv.coe_toRingHom, RingEquiv.apply_symm_apply] }
...
theorem quotientMap_injective' {J : Ideal R} {I : Ideal S} {f : R →+* S} {H : J ≤ I.comap f}
  ...
  rw [quotientMap_mk] at ha; rw [Quotient.eq_zero_iff_mem] at ha
...
def quotientMapₐ {I : Ideal A} (J : Ideal B) (f : A →ₐ[R₁] B) (hIJ : I ≤ J.comap f) :
    A ⧸ I →ₐ[R₁] B ⧸ J :=
  { quotientMap J (f : A →+* B) hIJ with commutes' := fun r => by simp only [RingHom.toFun_eq_coe, quotientMap_algebraMap, AlgHom.coe_toRingHom, AlgHom.commutes, Quotient.mk_algebraMap] }
...
def quotientEquivAlg (I : Ideal A) (J : Ideal B) (f : A ≃ₐ[R₁] B) (hIJ : J = I.map (f : A →+* B)) :
  ...
  { quotientEquiv I J (f : A ≃+* B) hIJ with
    commutes' := fun r => by
      ...
      rw [this]
      simp only [quotientEquiv_apply, RingHom.toFun_eq_coe, quotientMap_algebraMap, RingEquiv.coe_toRingHom, AlgEquiv.coe_ringEquiv, AlgEquiv.commutes, Quotient.mk_algebraMap]}
...
lemma comap_map_mk {I J : Ideal R} (h : I ≤ J) :
  ...
  by ext; rw [← Ideal.mem_quotient_iff_mem h]; rw [Ideal.mem_comap]
...
theorem ker_quotLeftToQuotSup : RingHom.ker (quotLeftToQuotSup I J) =
  ...
  simp only [mk_ker, sup_idem, sup_comm, quotLeftToQuotSup, Quotient.factor, ker_quotient_lift, map_eq_iff_sup_ker_eq_of_surjective (Ideal.Quotient.mk I) Quotient.mk_surjective, ← sup_assoc]
...
theorem ker_quotQuotMk : RingHom.ker (quotQuotMk I J) = I ⊔ J := by
  rw [RingHom.ker_eq_comap_bot]; rw [quotQuotMk]; rw [← comap_comap]; rw [← RingHom.ker]; rw [mk_ker]; rw [comap_map_of_surjective (Ideal.Quotient.mk I) Quotient.mk_surjective]; rw [← RingHom.ker]; rw [mk_ker]; rw [sup_comm]
