...
theorem Ideal.le_of_localization_maximal {I J : Ideal R}
  ...
  rw [← _root_.map_mul] at eq; rw [← sub_eq_zero] at eq; rw [← map_sub] at eq
...
theorem ideal_eq_bot_of_localization (I : Ideal R)
  ...
  ideal_eq_bot_of_localization' _ fun P hP =>
    (Ideal.map_eq_bot_iff_le_ker _).mpr fun x hx => by
      rw [RingHom.mem_ker]; rw [← Submodule.mem_bot R]; rw [← h P hP]; rw [IsLocalization.mem_coeSubmodule]
      exact ⟨x, hx, rfl⟩
#align ideal_eq_bot_of_localization ideal_eq_bot_of_localization
...
theorem localization_isReduced : LocalizationPreserves fun R hR => IsReduced R := by
  ...
  rw [← mul_left_comm] at hm'; rw [← pow_succ] at hm'; rw [← mul_pow] at hm'
...
theorem surjective_ofLocalizationSpan :
  ...
  introv R e H
  rw [← Set.range_iff_surjective]; rw [Set.eq_univ_iff_forall]
  letI := f.toAlgebra
  intro x
  ...
  rw [eq_comm] at e'; rw [IsLocalization.eq_mk'_iff_mul_eq] at e'; rw [Subtype.coe_mk] at e'; rw [Subtype.coe_mk] at e'; rw [← map_mul] at e'
  obtain ⟨⟨_, n', rfl⟩, e''⟩ := (IsLocalization.eq_iff_exists (Submonoid.powers (f r)) _).mp e'
  dsimp only at e''
  rw [mul_comm x] at e''; rw [← mul_assoc] at e''; rw [← map_pow] at e''; rw [← map_mul] at e''; rw [← map_mul] at e''; rw [← pow_add] at e''
  exact ⟨n' + n, _, e''.symm⟩
...
theorem localization_finite : RingHom.LocalizationPreserves @RingHom.Finite := by
  ...
  rw [IsLocalization.mk'_eq_mul_mk'_one]; rw [mul_comm]; rw [Finset.coe_image]
...
theorem multiple_mem_span_of_mem_localization_span [Algebra R' S] [Algebra R S]
  ...
  rw [smul_add]; rw [← smul_smul]; rw [mul_comm]; rw [← smul_smul]; rw [← smul_smul]; rw [← e]
  rfl
...
theorem finite_ofLocalizationSpan : RingHom.OfLocalizationSpan @RingHom.Finite := by
  ...
  obtain ⟨⟨_, n₁, rfl⟩, hn₁⟩ :=
    multiple_mem_span_of_mem_localization_span (Submonoid.powers (r : R))
      ...
      (by rw [s₂ r]; trivial)
  dsimp only at hn₁
  rw [Submonoid.smul_def] at hn₁; rw [Algebra.smul_def] at hn₁; rw [IsScalarTower.algebraMap_apply R S] at hn₁; rw [← map_mul] at hn₁
  obtain ⟨⟨_, n₂, rfl⟩, hn₂⟩ :=
    IsLocalization.smul_mem_finsetIntegerMultiple_span (Submonoid.powers (r : R))
      (Localization.Away (f r)) _ (s₁ r) hn₁
  rw [Submonoid.smul_def] at hn₂; rw [← Algebra.smul_def] at hn₂; rw [smul_smul] at hn₂; rw [Subtype.coe_mk] at hn₂; rw [← pow_add] at hn₂
...
theorem IsLocalization.exists_smul_mem_of_mem_adjoin [Algebra R S] [Algebra R S']
  ...
  rw [Algebra.smul_def] at hx''; rw [← _root_.map_mul] at hx''
