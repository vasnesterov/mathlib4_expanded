...
theorem prod_X_add_C_coeff (s : Multiset R) {k : ℕ} (h : k ≤ Multiset.card s) :
  ...
  · intro j hj1 hj2
    ...
    · intro hn
      rw [hn] at hj2; rw [Nat.sub_sub_self (Nat.lt_succ_iff.mp (Finset.mem_range.mp hj1))] at hj2
      exact Ne.irrefl hj2
  · rw [Finset.mem_range]
...
theorem esymm_neg (s : Multiset R) (k : ℕ) : (map Neg.neg s).esymm k = (-1) ^ k * esymm s k := by
  rw [esymm]; rw [esymm]; rw [← Multiset.sum_map_mul_left]; rw [Multiset.powersetLen_map]; rw [Multiset.map_map]; rw [map_congr (Eq.refl _)]
  intro x hx
  rw [(mem_powersetLen.mp hx).right.symm]; rw [← prod_replicate]; rw [← Multiset.map_const]
  nth_rw 3 [← map_id' x]
  rw [← prod_map_mul]; rw [map_congr (Eq.refl _)];rfl
