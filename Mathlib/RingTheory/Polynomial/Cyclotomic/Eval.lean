...
theorem eval_one_cyclotomic_prime {R : Type*} [CommRing R] {p : ℕ} [hn : Fact p.Prime] :
  ...
  simp only [cyclotomic_prime, eval_X, one_pow, Finset.sum_const, eval_pow, eval_finset_sum, Finset.card_range, smul_one_eq_coe]
...
theorem eval_one_cyclotomic_prime_pow {R : Type*} [CommRing R] {p : ℕ} (k : ℕ)
  ...
  simp only [cyclotomic_prime_pow_eq_geom_sum hn.out, eval_X, one_pow, Finset.sum_const, eval_pow, eval_finset_sum, Finset.card_range, smul_one_eq_coe]
...
private theorem cyclotomic_neg_one_pos {n : ℕ} (hn : 2 < n) {R} [LinearOrderedCommRing R] :
  ...
  rw [← map_cyclotomic_int]; rw [← Int.cast_one]; rw [← Int.cast_neg]; rw [eval_int_cast_map]; rw [Int.coe_castRingHom]; rw [Int.cast_pos]
  suffices 0 < eval (↑(-1 : ℤ)) (cyclotomic n ℝ) by
    rw [← map_cyclotomic_int n ℝ] at this; rw [eval_int_cast_map] at this; rw [Int.coe_castRingHom] at this
...
theorem cyclotomic_pos {n : ℕ} (hn : 2 < n) {R} [LinearOrderedCommRing R] (x : R) :
  ...
  rw [← cons_self_properDivisors hn'.ne'] at this; rw [Finset.erase_cons_of_ne _ hn''.ne'] at this; rw [Finset.prod_cons] at this; rw [eval_mul] at this; rw [eval_geom_sum] at this
...
theorem eval_one_cyclotomic_not_prime_pow {R : Type*} [Ring R] {n : ℕ}
  ...
  rw [eval_geom_sum] at this; rw [one_geom_sum] at this; rw [eval_prod] at this; rw [eq_comm] at this; rw [←
    Finset.prod_sdiff <| @range_pow_padicValNat_subset_divisors' p _ _] at this; rw [Finset.prod_image] at this
  ...
  swap
  · simp only [singleton_subset_iff, mem_sdiff, mem_erase, Ne.def, mem_divisors, dvd_refl, true_and_iff, mem_image, mem_range, exists_prop, not_exists, not_and]
    exact ⟨⟨hn.ne', hn'.ne'⟩, fun t _ => h hp _⟩
  rw [← Int.natAbs_ofNat p] at hpe; rw [Int.natAbs_dvd_natAbs] at hpe
  obtain ⟨t, ht⟩ := hpe
  rw [Finset.prod_singleton] at this; rw [ht] at this; rw [mul_left_comm] at this; rw [mul_comm] at this; rw [← mul_assoc] at this; rw [mul_assoc] at this
...
theorem sub_one_pow_totient_lt_cyclotomic_eval {n : ℕ} {q : ℝ} (hn' : 2 ≤ n) (hq' : 1 < q) :
  ...
  have hex : ∃ ζ' ∈ primitiveRoots n ℂ, q - 1 < ‖↑q - ζ'‖ := by
    ...
    rw [Complex.arg_ofReal_of_nonneg hq.le]; rw [Ne.def]; rw [eq_comm]; rw [hζ.arg_eq_zero_iff hn.ne']
    clear_value ζ
    rintro rfl
  ...
  suffices Units.mk0 (Real.toNNReal (q - 1)) (by simp [hq']) ^ totient n <
    ...
    simp only [← Units.val_lt_val, Units.val_pow_eq_pow_val, Units.val_mk0, ← NNReal.coe_lt_coe, hq'.le, Real.toNNReal_lt_toNNReal_iff_of_nonneg, coe_nnnorm, Complex.norm_eq_abs, NNReal.coe_pow, Real.coe_toNNReal', max_eq_left, sub_nonneg] at this
    convert this
  ...
  · simp only [Subtype.coe_mk, Finset.mem_attach, forall_true_left, Subtype.forall, ←
      Units.val_le_val, ← NNReal.coe_le_coe, Complex.abs.nonneg, hq'.le, Units.val_mk0, Real.coe_toNNReal', coe_nnnorm, Complex.norm_eq_abs, max_le_iff, tsub_le_iff_right]
    intro x hx
...
theorem cyclotomic_eval_lt_add_one_pow_totient {n : ℕ} {q : ℝ} (hn' : 3 ≤ n) (hq' : 1 < q) :
  ...
  have hex : ∃ ζ' ∈ primitiveRoots n ℂ, ‖↑q - ζ'‖ < q + 1 := by
    ...
    rw [Complex.arg_ofReal_of_nonneg hq.le]; rw [Ne.def]; rw [eq_comm]
    intro h
    rw [Complex.arg_eq_zero_iff] at h; rw [Complex.neg_re] at h; rw [neg_nonneg] at h; rw [Complex.neg_im] at h; rw [neg_eq_zero] at h
    have hζ₀ : ζ ≠ 0 := by
      clear_value ζ
      rintro rfl
      exact hn.ne' (hζ.unique IsPrimitiveRoot.zero)
    have : ζ.re < 0 ∧ ζ.im = 0 := ⟨h.1.lt_of_ne ?_, h.2⟩
    rw [← Complex.arg_eq_pi_iff] at this; rw [hζ.arg_eq_pi_iff hn.ne'] at this
    rw [this] at hζ
  ...
  · simp only [Subtype.coe_mk, Finset.mem_attach, forall_true_left, Subtype.forall, ←
      Units.val_le_val, ← NNReal.coe_le_coe, Complex.abs.nonneg, hq'.le, Units.val_mk0, Real.coe_toNNReal, coe_nnnorm, Complex.norm_eq_abs, max_le_iff]
    intro x hx
...
theorem sub_one_pow_totient_lt_natAbs_cyclotomic_eval {n : ℕ} {q : ℕ} (hn' : 1 < n) (hq : q ≠ 1) :
  ...
  · rw [zero_tsub, zero_pow (Nat.totient_pos (pos_of_gt hn')), pos_iff_ne_zero, Int.natAbs_ne_zero,
    ...
    exact one_ne_zero
  rw [← @Nat.cast_lt ℝ]; rw [Nat.cast_pow]; rw [Nat.cast_sub hq'.le]; rw [Nat.cast_one]; rw [Int.cast_natAbs]
