...
theorem natDegree_cyclotomic' {ζ : R} {n : ℕ} (h : IsPrimitiveRoot ζ n) :
  ...
  simp only [IsPrimitiveRoot.card_primitiveRoots h, mul_one, natDegree_X_sub_C, Nat.cast_id, Finset.sum_const, nsmul_eq_mul]
  intro z _
...
theorem X_pow_sub_one_eq_prod {ζ : R} {n : ℕ} (hpos : 0 < n) (h : IsPrimitiveRoot ζ n) :
  ...
  rw [nthRootsFinset]; rw [← Multiset.toFinset_eq (IsPrimitiveRoot.nthRoots_nodup h)]
...
theorem X_pow_sub_one_splits {ζ : K} {n : ℕ} (h : IsPrimitiveRoot ζ n) :
  ...
  rw [splits_iff_card_roots]; rw [← nthRoots]; rw [IsPrimitiveRoot.card_nthRoots h]; rw [natDegree_X_pow_sub_C]
set_option linter.uppercaseLean3 false in
...
theorem prod_cyclotomic'_eq_X_pow_sub_one {K : Type*} [CommRing K] [IsDomain K] {ζ : K} {n : ℕ}
  ...
  simp only [X_pow_sub_one_eq_prod hpos h, cyclotomic', ← Finset.prod_biUnion hd, h.nthRoots_one_eq_biUnion_primitiveRoots]
set_option linter.uppercaseLean3 false in
...
theorem int_cyclotomic_spec (n : ℕ) :
  ...
  by_cases hzero : n = 0
  · simp only [hzero, cyclotomic, degree_one, monic_one, cyclotomic'_zero, dif_pos, eq_self_iff_true, Polynomial.map_one, and_self_iff]
...
theorem degree_cyclotomic (n : ℕ) (R : Type*) [Ring R] [Nontrivial R] :
  ...
  simp only [(int_cyclotomic_spec n).right.right, eq_intCast, Monic.leadingCoeff, Int.cast_one, Ne.def, not_false_iff, one_ne_zero]
#align polynomial.degree_cyclotomic Polynomial.degree_cyclotomic
...
theorem degree_cyclotomic_pos (n : ℕ) (R : Type*) (hpos : 0 < n) [Ring R] [Nontrivial R] :
  ...
  rw [degree_cyclotomic n R]; rw [Nat.cast_pos]; exact Nat.totient_pos hpos
...
theorem prod_cyclotomic_eq_X_pow_sub_one {n : ℕ} (hpos : 0 < n) (R : Type*) [CommRing R] :
  ...
  have integer : ∏ i in Nat.divisors n, cyclotomic i ℤ = X ^ n - 1 := by
    apply map_injective (Int.castRingHom ℂ) Int.cast_injective
    simp only [Polynomial.map_prod, int_cyclotomic_spec, Polynomial.map_pow, map_X, Polynomial.map_one, Polynomial.map_sub]
...
theorem prod_cyclotomic_eq_geom_sum {n : ℕ} (h : 0 < n) (R) [CommRing R] :
  ...
  rw [← mul_left_inj' (cyclotomic_ne_zero 1 ℤ)]; rw [prod_erase_mul _ _ (Nat.one_mem_divisors.2 h.ne')]; rw [cyclotomic_one]; rw [geom_sum_mul]; rw [prod_cyclotomic_eq_X_pow_sub_one h]
...
theorem X_pow_sub_one_mul_prod_cyclotomic_eq_X_pow_sub_one_of_dvd (R) [CommRing R] {d n : ℕ}
  ...
  rw [← prod_cyclotomic_eq_X_pow_sub_one h0d]; rw [← prod_cyclotomic_eq_X_pow_sub_one h0n]; rw [mul_comm]; rw [Finset.prod_sdiff (Nat.divisors_subset_of_dvd h0n.ne' hd)]
set_option linter.uppercaseLean3 false in
...
theorem X_pow_sub_one_mul_cyclotomic_dvd_X_pow_sub_one_of_dvd (R) [CommRing R] {d n : ℕ}
  ...
  rw [mul_assoc]
  congr 1
  rw [← Nat.insert_self_properDivisors hdn.ne_bot]; rw [insert_sdiff_of_not_mem]; rw [prod_insert]
...
theorem cyclotomic_prime_pow_eq_geom_sum {R : Type*} [CommRing R] {p n : ℕ} (hp : p.Prime) :
  ...
  rw [this] at n_ih
  rw [mul_comm _ (∑ i in _, _)]; rw [n_ih]; rw [geom_sum_mul]; rw [sub_left_inj]; rw [← pow_mul]; rw [pow_add]; rw [pow_one]
...
theorem cyclotomic_coeff_zero (R : Type*) [CommRing R] {n : ℕ} (hn : 1 < n) :
  ...
  have hprod : (∏ i in Nat.properDivisors n, (Polynomial.cyclotomic i R).coeff 0) = -1 := by
    rw [← Finset.insert_erase (Nat.one_mem_properDivisors_iff_one_lt.2
      (lt_of_lt_of_le one_lt_two hn))]; rw [Finset.prod_insert (Finset.not_mem_erase 1 _)]; rw [cyclotomic_one R]
...
theorem coprime_of_root_cyclotomic {n : ℕ} (hpos : 0 < n) {p : ℕ} [hprime : Fact p.Prime] {a : ℕ}
  ...
  rw [IsRoot.def] at hroot; rw [eq_natCast] at hroot; rw [h] at hroot; rw [← coeff_zero_eq_eval_zero] at hroot
  by_cases hone : n = 1
  · simp only [hone, cyclotomic_one, zero_sub, coeff_one_zero, coeff_X_zero, neg_eq_zero, one_ne_zero, coeff_sub] at hroot
