...
theorem weightedTotalDegree_coe (w : σ → M) (p : MvPolynomial σ R) (hp : p ≠ 0) :
  ...
  rw [Ne.def] at hp; rw [← weightedTotalDegree'_eq_bot_iff w p] at hp; rw [← Ne.def] at hp; rw [WithBot.ne_bot_iff_exists] at hp
  obtain ⟨m, hm⟩ := hp
  apply le_antisymm
...
theorem weightedHomogeneousSubmodule_eq_finsupp_supported (w : σ → M) (m : M) :
  ...
  ext x
  rw [mem_supported]; rw [Set.subset_def]
...
theorem weightedHomogeneousSubmodule_mul (w : σ → M) (m n : M) :
  ...
  rw [← Finsupp.mem_antidiagonal.mp hde]; rw [← hφ aux.1]; rw [← hψ aux.2]; rw [map_add]
...
theorem sum_weightedHomogeneousComponent :
  ...
  · intro hm
    ...
    rw [hm] at this; rw [if_pos rfl] at this; rw [coeff_zero] at this
    exact this.symm
...
theorem weightedHomogeneousComponent_zero [NoZeroSMulDivisors ℕ M] (hw : ∀ i : σ, w i ≠ 0) :
  ...
  · rw [coeff_weightedHomogeneousComponent, if_neg, coeff_C, if_neg (Ne.symm hd)]
    simp only [weightedDegree', LinearMap.toAddMonoidHom_coe, Finsupp.total_apply, Finsupp.sum, sum_eq_zero_iff, Finsupp.mem_support_iff, Ne.def, smul_eq_zero, not_forall, not_or, and_self_left, exists_prop]
