...
theorem trace_comp_trace [Algebra K T] [Algebra L T] [IsScalarTower K L T] [FiniteDimensional K L]
  ...
  ext; rw [LinearMap.comp_apply]; rw [LinearMap.restrictScalars_apply]; rw [trace_trace]
...
theorem traceForm_toMatrix [DecidableEq ι] (i j) :
  ...
  rw [BilinForm.toMatrix_apply]; rw [traceForm_apply]
...
theorem traceForm_toMatrix_powerBasis (h : PowerBasis R S) :
  ...
  by ext; rw [traceForm_toMatrix]; rw [of_apply]; rw [pow_add]; rw [h.basis_eq_pow]; rw [h.basis_eq_pow]
...
theorem PowerBasis.trace_gen_eq_nextCoeff_minpoly [Nontrivial S] (pb : PowerBasis K S) :
  ...
  rw [trace_eq_matrix_trace pb.basis]; rw [trace_eq_neg_charpoly_coeff]; rw [charpoly_leftMulMatrix]; rw [←
    pb.natDegree_minpoly]; rw [Fintype.card_fin]; rw [← nextCoeff_of_pos_natDegree _ d_pos']
...
theorem PowerBasis.trace_gen_eq_sum_roots [Nontrivial S] (pb : PowerBasis K S)
  ...
  rw [PowerBasis.trace_gen_eq_nextCoeff_minpoly]; rw [RingHom.map_neg]; rw [←
    nextCoeff_map (algebraMap K F).injective]; rw [sum_roots_eq_nextCoeff_of_monic_of_split ((minpoly.monic (PowerBasis.isIntegral_gen _)).map _)
      ((splits_id_iff_splits _).2 hf)]; rw [neg_neg]
...
theorem trace_gen_eq_sum_roots (x : L) (hf : (minpoly K x).Splits (algebraMap K F)) :
  ...
  rw [← adjoin.powerBasis_gen hx]; rw [(adjoin.powerBasis hx).trace_gen_eq_sum_roots]; all_goals
    rw [adjoin.powerBasis_gen hx]; rw [← minpoly.algebraMap_eq injKxL]; all_goals
...
-- and it was after the first `rw`.
  ...
  rw [← @trace_trace _ _ K K⟮x⟯ _ _ _ _ _ _ _ _ _]; rw [trace_algebraMap]; rw [LinearMap.map_smul_of_tower]
...
theorem trace_eq_sum_roots [FiniteDimensional K L] {x : L}
  ...
  rw [trace_eq_trace_adjoin K x]; rw [Algebra.smul_def]; rw [RingHom.map_mul]; rw [← Algebra.smul_def]; rw [IntermediateField.AdjoinSimple.trace_gen_eq_sum_roots _ hF]
...
theorem trace_eq_sum_embeddings [FiniteDimensional K L] [IsSeparable K L] {x : L} :
  ...
  rw [trace_eq_trace_adjoin K x]; rw [Algebra.smul_def]; rw [RingHom.map_mul]; rw [← adjoin.powerBasis_gen hx]; rw [trace_eq_sum_embeddings_gen E pb (IsAlgClosed.splits_codomain _)]
...
theorem traceMatrix_of_basis_mulVec (b : Basis ι A B) (z : B) :
  ...
  ext i
  rw [← col_apply ((traceMatrix A b).mulVec (b.equivFun z)) i Unit.unit]; rw [col_mulVec]; rw [Matrix.mul_apply]; rw [traceMatrix]
  simp only [col_apply, traceForm_apply]
  conv_lhs =>
    congr
    rfl
    ext
    rw [mul_comm _ (b.equivFun z _)]; rw [← smul_eq_mul]; rw [of_apply]; rw [← LinearMap.map_smul]
  rw [← _root_.map_sum]
  congr
  conv_lhs =>
    congr
...
theorem det_traceMatrix_ne_zero' [IsSeparable K L] : det (traceMatrix K pb.basis) ≠ 0 := by
  ...
  rw [RingHom.map_det]; rw [RingHom.mapMatrix_apply]; rw [traceMatrix_eq_embeddingsMatrixReindex_mul_trans K _ _ e]; rw [embeddingsMatrixReindex_eq_vandermonde]; rw [det_mul]; rw [det_transpose]
...
theorem det_traceForm_ne_zero [IsSeparable K L] [DecidableEq ι] (b : Basis ι K L) :
  ...
  rw [← BilinForm.toMatrix_mul_basis_toMatrix pb.basis b]; rw [←
