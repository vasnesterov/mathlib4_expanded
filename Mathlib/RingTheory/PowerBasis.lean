...
theorem finrank [Algebra K S] (pb : PowerBasis K S) : FiniteDimensional.finrank K S = pb.dim := by
  rw [FiniteDimensional.finrank_eq_card_basis pb.basis]; rw [Fintype.card_fin]
...
theorem aeval_minpolyGen (pb : PowerBasis A S) : aeval pb.gen (minpolyGen pb) = 0 := by
  ...
  rw [Finsupp.total_apply]; rw [Finsupp.sum_fintype]; all_goals
...
theorem constr_pow_aeval (pb : PowerBasis A S) {y : S'} (hy : aeval y (minpoly A pb.gen) = 0)
  ...
  rw [← aeval_modByMonic_eq_self_of_root (minpoly.monic pb.isIntegral_gen) (minpoly.aeval _ _)]; rw [←
  ...
  congr
  rw [← Fin.val_mk hi]; rw [← pb.basis_eq_pow ⟨i, hi⟩]; rw [Basis.constr_basis]
...
theorem minpolyGen_map (pb : PowerBasis A S) (e : S ≃ₐ[A] S') :
  ...
  simp only [LinearEquiv.trans_apply, map_basis, Basis.map_repr, map_gen, AlgEquiv.toLinearEquiv_apply, e.toLinearEquiv_symm, AlgEquiv.map_pow, AlgEquiv.symm_apply_apply, sub_right_inj]
