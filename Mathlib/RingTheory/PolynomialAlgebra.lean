...
theorem toFunBilinear_apply_eq_sum (a : A) (p : R[X]) :
  ...
  congr with i : 1
  rw [← Algebra.smul_def]; rw [← C_mul']; rw [mul_smul_comm]; rw [C_mul_X_pow_eq_monomial]; rw [← Algebra.commutes]; rw [← Algebra.smul_def]; rw [smul_monomial]
...
theorem right_inv (x : A[X]) : (toFunAlgHom R A) (invFun R A x) = x := by
  ...
  · intro n a
    rw [invFun_monomial]; rw [Algebra.TensorProduct.tmul_pow]; rw [one_pow]; rw [Algebra.TensorProduct.tmul_mul_tmul]; rw [mul_one]; rw [one_mul]; rw [toFunAlgHom_apply_tmul]; rw [X_pow_eq_monomial]; rw [sum_monomial_index]; all_goals
      simp
...
theorem matPolyEquiv_coeff_apply_aux_1 (i j : n) (k : ℕ) (x : R) :
  ...
  simp only [Algebra.TensorProduct.tmul_mul_tmul, one_pow, one_mul, Matrix.mul_one, Algebra.TensorProduct.tmul_pow, Algebra.TensorProduct.includeLeft_apply]
...
theorem support_subset_support_matPolyEquiv (m : Matrix n n R[X]) (i j : n) :
  ...
  intro hk
  rw [← matPolyEquiv_coeff_apply]; rw [hk]
  rfl
