...
theorem irreducible_of_span_eq_maximalIdeal {R : Type*} [CommRing R] [LocalRing R] [IsDomain R]
  ...
  rw [h] at ha hb; rw [mem_span_singleton'] at ha hb
...
theorem iff_pid_with_one_nonzero_prime (R : Type u) [CommRing R] [IsDomain R] :
  ...
  · rintro ⟨RPID, Punique⟩
    ...
    intro h
    rw [h] at hPM; rw [le_bot_iff] at hPM
    exact hP1 hPM
...
theorem toUniqueFactorizationMonoid : UniqueFactorizationMonoid R :=
  ...
  UniqueFactorizationMonoid.of_exists_prime_factors fun x hx => by
    ...
    · intro q hq
      ...
      rw [mul_assoc] at h; rw [mul_left_comm] at h; rw [IsUnit.dvd_mul_left _ _ _ (Units.isUnit _)] at h
...
theorem ideal_eq_span_pow_irreducible {s : Ideal R} (hs : s ≠ ⊥) {ϖ : R} (hirr : Irreducible ϖ) :
  ...
  have gen_ne_zero : generator s ≠ 0 := by
    rw [Ne.def]; rw [← eq_bot_iff_generator_eq_zero]
    assumption
...
theorem unit_mul_pow_congr_unit {ϖ : R} (hirr : Irreducible ϖ) (u v : Rˣ) (m n : ℕ)
  ...
  rw [← sub_eq_zero] at h
  rw [← sub_mul] at h; rw [mul_eq_zero] at h
  cases' h with h h
  · rw [sub_eq_zero] at h
    exact_mod_cast h
...
theorem addVal_def (r : R) (u : Rˣ) {ϖ : R} (hϖ : Irreducible ϖ) (n : ℕ) (hr : r = u * ϖ ^ n) :
    addVal R r = n := by
  rw [addVal]; rw [addValuation_apply]; rw [hr]; rw [eq_of_associated_left
      (associated_of_irreducible R hϖ (Classical.choose_spec (exists_prime R)).irreducible)]; rw [eq_of_associated_right (Associated.symm ⟨u, mul_comm _ _⟩)]; rw [multiplicity_pow_self_of_prime (PrincipalIdealRing.irreducible_iff_prime.1 hϖ)]
...
theorem addVal_le_iff_dvd {a b : R} : addVal R a ≤ addVal R b ↔ a ∣ b := by
  ...
  · by_cases ha0 : a = 0
    ...
    rw [addVal] at h; rw [addValuation_apply] at h; rw [addValuation_apply] at h; rw [multiplicity_le_multiplicity_iff] at h
...
instance (R : Type*) [CommRing R] [IsDomain R] [DiscreteValuationRing R] :
  ...
  haus' x hx := by
    ...
    simp only [← Ideal.one_eq_top, smul_eq_mul, mul_one, SModEq.zero, hϖ.maximalIdeal_eq, Ideal.span_singleton_pow, Ideal.mem_span_singleton, ← addVal_le_iff_dvd, hϖ.addVal_pow] at hx
