...
theorem of_restrict_scalars_finitePresentation [Algebra A B] [IsScalarTower R A B]
  ...
  · obtain ⟨t, ht⟩ := hRA.out
    ...
    rw [map_add] at hx; rw [aeval_map_algebraMap] at hx; rw [← aeval_unique] at hx; rw [show MvPolynomial.aeval (f ∘ X) q = 0
      from leI hq] at hx; rw [add_zero] at hx
...
theorem ker_fg_of_mvPolynomial {n : ℕ} (f : MvPolynomial (Fin n) R →ₐ[R] A)
  ...
  classical
    ...
    refine' add_mem _ hy
    simp only [RingHom.mem_ker, AlgHom.toRingHom_eq_coe, AlgHom.coe_toRingHom, map_add, show f y = 0 from leI hy, add_zero, hh'] at hx
