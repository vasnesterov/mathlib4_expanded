...
theorem isReduced_zmod {n : ℕ} : IsReduced (ZMod n) ↔ Squarefree n ∨ n = 0 := by
  rw [← RingHom.ker_isRadical_iff_reduced_of_surjective
      (ZMod.ringHom_surjective <| Int.castRingHom <| ZMod n)]; rw [ZMod.ker_int_castRingHom]; rw [← isRadical_iff_span_singleton]; rw [isRadical_iff_squarefree_or_zero]; rw [Int.squarefree_coe_nat]; rw [Nat.cast_eq_zero]
