...
lemma polynomial_expand_eq (f : R[X]) :
  ...
  rw [← (f.map (S := R) (frobeniusEquiv R p).symm).expand_char p]; rw [map_expand]; rw [map_map]; rw [frobenius_comp_frobeniusEquiv_symm]; rw [map_id]
...
instance toPerfectRing (p : ℕ) [hp : Fact p.Prime] [CharP K p] : PerfectRing K p := by
  ...
  have hf_deg : f.degree ≠ 0 := by
    rw [degree_X_pow_sub_C hp.out.pos y]; rw [p.cast_ne_zero]; exact hp.out.ne_zero
  ...
  · rw [g.natDegree_map, ← degree_eq_iff_natDegree_eq_of_pos Nat.one_pos] at this
    ...
    rw [RingHom.map_frobenius]; rw [ha']; rw [frobenius_def]; rw [ha_pow]
  have hg_dvd : g.map ι ∣ (X - C a) ^ p := by
    convert Polynomial.map_dvd ι (minpoly.dvd K a hfa)
    rw [sub_pow_char]; rw [Polynomial.map_sub]; rw [Polynomial.map_pow]; rw [map_X]; rw [map_C]; rw [← ha_pow]; rw [map_pow]
  ...
  have hg_pow : g.map ι = (X - C a) ^ (g.map ι).natDegree := by
    ...
    rw [eq_of_monic_of_associated ((minpoly.monic ha).map ι) ((monic_X_sub_C a).pow q) hq]; rw [natDegree_pow]; rw [natDegree_X_sub_C]; rw [mul_one]
  ...
  rw [g.natDegree_map ι]; rw [← Nat.pos_iff_ne_zero]; rw [natDegree_pos_iff_degree_pos]
