...
theorem norm_sub_modPart_aux (r : ℚ) (h : ‖(r : ℚ_[p])‖ ≤ 1) :
  ...
  simp only [Int.cast_ofNat, CharP.cast_eq_zero, EuclideanDomain.mod_zero, Int.cast_add, Int.cast_mul, zero_mul, add_zero] at this
  push_cast
  ...
  rw [← Int.coe_nat_dvd]; rw [← norm_int_lt_one_iff_dvd]; rw [not_lt]
  apply ge_of_eq
  rw [← isUnit_iff]
...
theorem zmod_congr_of_sub_mem_span_aux (n : ℕ) (x : ℤ_[p]) (a b : ℤ)
  ...
  rw [← sub_eq_zero]; rw [← Int.cast_sub]; rw [ZMod.int_cast_zmod_eq_zero_iff_dvd]; rw [Int.coe_nat_pow]
  rw [← dvd_neg] at ha; rw [neg_sub] at ha
  have := dvd_add ha hb
...
theorem dvd_appr_sub_appr (x : ℤ_[p]) (m n : ℕ) (h : m ≤ n) : p ^ m ∣ x.appr n - x.appr m := by
  ...
  rw [Nat.succ_eq_add_one]; rw [← add_assoc]
  dsimp [appr]
  split_ifs with h
  · exact ih
...
theorem appr_spec (n : ℕ) : ∀ x : ℤ_[p], x - appr x n ∈ Ideal.span {(p : ℤ_[p]) ^ n} := by
  ...
  rw [valuation_p_pow_mul _ _ hc']; rw [add_sub_cancel']; rw [_root_.pow_succ']; rw [← mul_sub]
  apply mul_dvd_mul_left
  obtain hc0 | hc0 := c.valuation.natAbs.eq_zero_or_pos
  · simp only [hc0, mul_one, _root_.pow_zero]
    rw [mul_comm] at hc; rw [unitCoeff_spec h] at hc
    ...
    obtain ⟨c, rfl⟩ : IsUnit c := by
      ...
      rw [isUnit_iff]; rw [norm_eq_pow_val hc']; rw [hc0]; rw [neg_zero]; rw [zpow_zero]
...
theorem zmod_cast_comp_toZModPow (m n : ℕ) (h : m ≤ n) :
  ...
  dsimp
  rw [ZMod.cast_nat_cast (pow_dvd_pow p h)]; rw [zmod_congr_of_sub_mem_span m (x.appr n) (x.appr n) (x.appr m)]
  · rw [sub_self]
...
theorem pow_dvd_nthHom_sub (r : R) (i j : ℕ) (h : i ≤ j) :
  ...
  rw [← Int.coe_nat_pow]; rw [← ZMod.int_cast_zmod_eq_zero_iff_dvd]; rw [Int.cast_sub]
  dsimp [nthHom]
...
theorem lift_spec (n : ℕ) : (toZModPow n).comp (lift f_compat) = f n := by
  ext r
  rw [RingHom.comp_apply]; rw [← ZMod.nat_cast_zmod_val (f n r)]; rw [← map_natCast <| toZModPow n]; rw [←
    sub_eq_zero]; rw [← RingHom.map_sub]; rw [← RingHom.mem_ker]; rw [ker_toZModPow]
...
theorem lift_unique (g : R →+* ℤ_[p]) (hg : ∀ n, (toZModPow n).comp g = f n) :
  ...
  rw [dist_eq_norm]; rw [norm_le_pow_iff_mem_span_pow]; rw [← ker_toZModPow]; rw [RingHom.mem_ker]; rw [RingHom.map_sub]; rw [← RingHom.comp_apply]; rw [← RingHom.comp_apply]; rw [lift_spec]; rw [hg]; rw [sub_self]
