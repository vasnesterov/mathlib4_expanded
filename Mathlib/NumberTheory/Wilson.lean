...
theorem wilsons_lemma : ((p - 1)! : ZMod p) = -1 := by
  ...
  · intro a ha
    rw [mem_Ico]; rw [← Nat.succ_sub hp]; rw [Nat.succ_sub_one]
    constructor
  ...
  · intro b hb
    rw [mem_Ico] at hb; rw [Nat.succ_le_iff] at hb; rw [← succ_sub hp] at hb; rw [succ_sub_one] at hb; rw [pos_iff_ne_zero] at hb
...
theorem prod_Ico_one_prime : ∏ x in Ico 1 p, (x : ZMod p) = -1 := by
  ...
  conv =>
    congr
    congr
    rw [← succ_sub_one p]; rw [succ_sub (Fact.out (p := p.Prime)).pos]
...
theorem prime_of_fac_equiv_neg_one (h : ((n - 1)! : ZMod n) = -1) (h1 : n ≠ 1) : Prime n := by
  ...
  rw [← ZMod.nat_cast_zmod_eq_zero_iff_dvd]; rw [cast_add]; rw [cast_one]; rw [h]; rw [add_left_neg]
