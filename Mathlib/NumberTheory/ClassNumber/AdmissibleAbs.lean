...
theorem exists_partition_int (n : ℕ) {ε : ℝ} (hε : 0 < ε) {b : ℤ} (hb : b ≠ 0) (A : Fin n → ℤ) :
  ...
  · rw [← ofNat_lt, natAbs_of_nonneg (hfloor i), floor_lt]
    ...
    rw [Algebra.smul_def]; rw [eq_intCast]; rw [← div_div]; rw [div_lt_div_right hε]; rw [div_lt_iff hb']; rw [one_mul]; rw [cast_lt]
    exact Int.emod_lt _ hb
  intro i₀ i₁ hi
  have hi : (⌊↑(A i₀ % b) / abs b • ε⌋.natAbs : ℤ) = ⌊↑(A i₁ % b) / abs b • ε⌋.natAbs :=
    congr_arg ((↑) : ℕ → ℤ) (Fin.mk_eq_mk.mp hi)
  rw [natAbs_of_nonneg (hfloor i₀)] at hi; rw [natAbs_of_nonneg (hfloor i₁)] at hi
  have hi := abs_sub_lt_one_of_floor_eq_floor hi
  rw [abs_sub_comm] at hi; rw [← sub_div] at hi; rw [abs_div] at hi; rw [abs_of_nonneg hbε.le] at hi; rw [div_lt_iff hbε] at hi; rw [one_mul] at hi
