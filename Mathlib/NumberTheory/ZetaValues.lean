...
theorem bernoulliFourierCoeff_recurrence (k : ℕ) {n : ℤ} (hn : n ≠ 0) :
  ...
  rw [QuotientAddGroup.mk_zero]; rw [fourier_eval_zero]; rw [one_mul]; rw [← ofReal_sub]; rw [bernoulliFun_eval_one]; rw [add_sub_cancel']
  congr 2
...
theorem summable_bernoulli_fourier {k : ℕ} (hk : 2 ≤ k) :
  ...
  have : (fun x : ℤ => ‖1 / (x : ℂ) ^ k‖) = fun x : ℤ => |1 / (x : ℝ) ^ k| := by
    ext1 x
    rw [norm_eq_abs]; rw [← Complex.abs_ofReal]
    congr 1
    norm_cast
  simp_rw [this]
  rw [summable_abs_iff]
...
theorem hasSum_one_div_pow_mul_fourier_mul_bernoulliFun {k : ℕ} (hk : 2 ≤ k) {x : ℝ}
  ...
  suffices ∀ {y : ℝ}, y ∈ Ico (0 : ℝ) 1 →
    ...
    rw [← Ico_insert_right (zero_le_one' ℝ)] at hx; rw [mem_insert_iff] at hx; rw [or_comm] at hx
  ...
  rw [smul_eq_mul]; rw [← mul_assoc]; rw [mul_div]; rw [mul_neg]; rw [div_mul_cancel]; rw [neg_neg]; rw [mul_pow _ (n : ℂ)]; rw [← div_div]; rw [div_self]
...
theorem hasSum_one_div_nat_pow_mul_cos {k : ℕ} (hk : k ≠ 0) {x : ℝ} (hx : x ∈ Icc (0 : ℝ) 1) :
  ...
  · convert (ofReal_re _).symm
    ...
    · rw [ofReal_cos, ofReal_mul, fourier_coe_apply, fourier_coe_apply, cos, ofReal_one, div_one,
      ...
      · ring
  · convert (ofReal_re _).symm
    rw [ofReal_mul]; rw [ofReal_div]; rw [ofReal_div]; rw [ofReal_mul]; rw [ofReal_pow]; rw [ofReal_pow]; rw [ofReal_neg]; rw [ofReal_nat_cast]; rw [ofReal_mul]; rw [ofReal_two]; rw [ofReal_one]
    rw [bernoulliFun]
    ring
...
theorem hasSum_one_div_nat_pow_mul_sin {k : ℕ} (hk : k ≠ 0) {x : ℝ} (hx : x ∈ Icc (0 : ℝ) 1) :
  ...
  have :
    ...
    · ext1 n
      rw [pow_add (-1 : ℂ)]; rw [pow_mul (-1 : ℂ)]; rw [neg_one_sq]; rw [one_pow]; rw [one_mul]; rw [pow_one]; rw [←
  ...
  · convert (ofReal_re _).symm
    ...
    · rw [ofReal_sin, ofReal_mul, fourier_coe_apply, fourier_coe_apply, sin, ofReal_one, div_one,
      ...
      · ring
  · convert (ofReal_re _).symm
    rw [ofReal_mul]; rw [ofReal_div]; rw [ofReal_div]; rw [ofReal_mul]; rw [ofReal_pow]; rw [ofReal_pow]; rw [ofReal_neg]; rw [ofReal_nat_cast]; rw [ofReal_mul]; rw [ofReal_two]; rw [ofReal_one]; rw [← div_div]; rw [div_I]; rw [div_mul_eq_mul_div₀]
