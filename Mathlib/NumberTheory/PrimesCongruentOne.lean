...
theorem exists_prime_gt_modEq_one {k : ℕ} (n : ℕ) (hk0 : k ≠ 0) :
  ...
  have hroot : IsRoot (cyclotomic k (ZMod p)) (castRingHom (ZMod p) b) := by
    ...
    rw [IsRoot.def]; rw [← map_cyclotomic_int k (ZMod p)]; rw [eval_map]; rw [coe_castRingHom]; rw [← Int.cast_ofNat]; rw [this]; rw [eval₂_hom]; rw [Int.coe_castRingHom]; rw [ZMod.int_cast_zmod_eq_zero_iff_dvd _ _]
