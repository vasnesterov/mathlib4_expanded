...
theorem quadraticChar_neg_two [DecidableEq F] (hF : ringChar F ≠ 2) :
  ...
  rw [(by norm_num : (-2 : F) = -1 * 2)]; rw [map_mul]; rw [χ₈'_eq_χ₄_mul_χ₈]; rw [quadraticChar_neg_one hF]; rw [quadraticChar_two hF]; rw [@cast_nat_cast _ (ZMod 4) _ _ _ (by norm_num : 4 ∣ 8)]
...
theorem FiniteField.isSquare_odd_prime_iff (hF : ringChar F ≠ 2) {p : ℕ} [Fact p.Prime]
  ...
  · rw [show (p : F) = 0 by rw [← hFp]; exact ringChar.Nat.cast_ringChar]
    ...
    conv => enter [1, 1, 2]; rw [hc]; rw [Nat.cast_pow]; rw [map_pow]; rw [hchar]; rw [map_ringChar]
