...
theorem bernoulli_def (n : ℕ) : bernoulli n =
  ...
  apply sum_congr rfl
  rintro x hx
  rw [mem_range_succ_iff] at hx; rw [choose_symm hx]; rw [tsub_tsub_cancel_of_le hx]
...
theorem bernoulli_eval_zero (n : ℕ) : (bernoulli n).eval 0 = _root_.bernoulli n := by
  rw [bernoulli]; rw [eval_finset_sum]; rw [sum_range_succ]
...
theorem bernoulli_eval_one (n : ℕ) : (bernoulli n).eval 1 = bernoulli' n := by
  ...
  simp only [← succ_eq_add_one, sum_range_succ, mul_one, cast_one, choose_self, (_root_.bernoulli _).mul_comm, sum_bernoulli, one_pow, mul_one, eval_C, eval_monomial, one_mul]
  by_cases h : n = 1
  · norm_num [h]
  · simp [h]
...
theorem derivative_bernoulli_add_one (k : ℕ) :
  ...
  rw [range_add_one]; rw [sum_insert not_mem_range_self]; rw [tsub_self]; rw [cast_zero]; rw [mul_zero]; rw [map_zero]; rw [zero_add]; rw [mul_sum]
...
nonrec theorem sum_bernoulli (n : ℕ) :
  ...
  simp only [add_right_eq_self, mul_one, cast_one, cast_add, add_tsub_cancel_left, choose_succ_self_right, one_smul, _root_.bernoulli_zero, sum_singleton, zero_add, map_add, range_one, bernoulli_zero, mul_one, one_mul, add_zero, choose_self]
  apply sum_eq_zero fun x hx => _
...
theorem bernoulli_eq_sub_sum (n : ℕ) :
  ...
  rw [Nat.cast_succ]; rw [← sum_bernoulli n]; rw [sum_range_succ]; rw [add_sub_cancel']; rw [choose_succ_self_right]; rw [Nat.cast_succ]
...
theorem sum_range_pow_eq_bernoulli_sub (n p : ℕ) :
  ...
  rw [sum_range_pow]; rw [bernoulli_def]; rw [eval_finset_sum]; rw [← sum_div]; rw [mul_div_cancel' _ _]
...
theorem bernoulli_eval_one_add (n : ℕ) (x : ℚ) :
  ...
  rw [← smul_eq_mul]; rw [← eval_smul]; rw [bernoulli_eq_sub_sum]; rw [mul_add]; rw [← smul_eq_mul]; rw [← eval_smul]; rw [bernoulli_eq_sub_sum]; rw [eval_sub]; rw [eval_finset_sum]
  conv_lhs =>
    congr
    · skip
    · apply_congr
  ...
  rw [eval_sub]; rw [eval_finset_sum]
  simp_rw [eval_smul, smul_add]
  rw [sum_add_distrib]; rw [sub_add]; rw [sub_eq_sub_iff_sub_eq_sub]; rw [_root_.add_sub_sub_cancel]
  conv_rhs =>
    congr
    · skip
    · congr
      rw [succ_eq_add_one]; rw [← choose_succ_self_right d]
  rw [Nat.cast_succ]; rw [← smul_eq_mul]; rw [← sum_range_succ _ d]; rw [eval_monomial_one_add_sub]
  simp_rw [smul_eq_mul]
...
theorem bernoulli_generating_function (t : A) :
  ...
  simp only [RingHom.map_sub, tsub_self, constantCoeff_one, constantCoeff_exp, coeff_zero_eq_constantCoeff, mul_zero, sub_self, add_zero]
  ...
  rw [mul_left_comm]; rw [← RingHom.map_mul]
  ...
  rw [← sum_bernoulli]; rw [Finset.mul_sum]; rw [AlgHom.map_sum]
  ...
  simp only [Nat.cast_choose ℚ (mem_range_le hi), coeff_mk, if_neg (mem_range_sub_ne_zero hi), one_div, AlgHom.map_smul, PowerSeries.coeff_one, coeff_exp, sub_zero, LinearMap.map_sub, Algebra.smul_mul_assoc, Algebra.smul_def, mul_right_comm _ ((aeval t) _), ← mul_assoc, ←
    RingHom.map_mul, succ_eq_add_one, ← Polynomial.C_eq_algebraMap, Polynomial.aeval_mul, Polynomial.aeval_C]
