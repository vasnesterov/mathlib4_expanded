...
theorem dvd_geom_sum₂_iff_of_dvd_sub {x y p : R} (h : p ∣ x - y) :
  ...
  simp only [← mem_span_singleton, ← eq_zero_iff_mem, RingHom.map_geom_sum₂, h, geom_sum₂_self, _root_.map_mul, map_pow, map_natCast]
...
theorem sq_dvd_add_pow_sub_sub (p x : R) (n : ℕ) :
  ...
  · simp only [Nat.succ_sub_succ_eq_sub, tsub_zero, Nat.cast_succ, add_pow, Finset.sum_range_succ, Nat.choose_self, Nat.succ_sub _, tsub_self, pow_one, Nat.choose_succ_self_right, pow_zero, mul_one, Nat.cast_zero, zero_add, Nat.succ_eq_add_one, add_tsub_cancel_left]
...
theorem pow_sub_pow_of_prime {p : R} (hp : Prime p) {x y : R} (hxy : p ∣ x - y) (hx : ¬p ∣ x)
  ...
  rw [← geom_sum₂_mul]; rw [multiplicity.mul hp]; rw [multiplicity_eq_zero.2 (not_dvd_geom_sum₂ hp hxy hx hn)]; rw [zero_add]
...
theorem pow_prime_sub_pow_prime :
  ...
  rw [← geom_sum₂_mul]; rw [multiplicity.mul hp]; rw [geom_sum₂_eq_one hp hp1 hxy hx]; rw [add_comm]
...
theorem pow_prime_pow_sub_pow_prime_pow (a : ℕ) :
  ...
  rw [← Nat.add_one]; rw [Nat.cast_add]; rw [Nat.cast_one]; rw [← add_assoc]; rw [← h_ind]; rw [pow_succ']; rw [pow_mul]; rw [pow_mul]
...
theorem Int.pow_sub_pow {x y : ℤ} (hxy : ↑p ∣ x - y) (hx : ¬↑p ∣ x) (n : ℕ) :
  ...
  rw [pow_sub_pow_of_prime hp]; rw [pow_prime_pow_sub_pow_prime_pow hp hp1 hxy hx]; rw [PartENat.natCast_get]
  · rw [← geom_sum₂_mul]
    exact dvd_mul_of_dvd_right hxy _
  · exact fun h => hx (hp.dvd_of_dvd_pow h)
  · rw [Int.coe_nat_dvd]
...
theorem Nat.pow_sub_pow {x y : ℕ} (hxy : p ∣ x - y) (hx : ¬p ∣ x) (n : ℕ) :
  ...
  · iterate 2 rw [← Int.coe_nat_multiplicity]
    ...
    push_cast at *
    exact Int.pow_sub_pow hp hp1 hxy hx n
  · simp only [Nat.sub_eq_zero_iff_le.mpr hyx, Nat.sub_eq_zero_iff_le.mpr (Nat.pow_le_pow_of_le_left hyx n), multiplicity.zero, PartENat.top_add]
...
theorem Int.sq_mod_four_eq_one_of_odd {x : ℤ} : Odd x → x ^ 2 % 4 = 1 := by
  ...
  ring_nf
  rw [add_assoc]; rw [← add_mul]; rw [Int.add_mul_emod_self]
  norm_num
...
theorem Int.two_pow_two_pow_sub_pow_two_pow {x y : ℤ} (n : ℕ) (hxy : 4 ∣ x - y) (hx : ¬2 ∣ x) :
  ...
  simp only [pow_two_pow_sub_pow_two_pow n, multiplicity.mul Int.prime_two, multiplicity.Finset.prod Int.prime_two, add_comm, Nat.cast_one, Finset.sum_const, Finset.card_range, nsmul_one, Int.two_pow_two_pow_add_two_pow_two_pow hx hxy]
...
theorem Int.two_pow_sub_pow' {x y : ℤ} (n : ℕ) (hxy : 4 ∣ x - y) (hx : ¬2 ∣ x) :
  ...
  rw [hk]; rw [pow_mul]; rw [pow_mul]; rw [multiplicity.pow_sub_pow_of_prime]; rw [Int.two_pow_two_pow_sub_pow_two_pow _ hxy hx]; rw [← hk]; rw [PartENat.natCast_get]
  · norm_cast
  · exact Int.prime_two
...
theorem Int.two_pow_sub_pow {x y : ℤ} {n : ℕ} (hxy : 2 ∣ x - y) (hx : ¬2 ∣ x) (hn : Even n) :
  ...
  have hy : Odd y := by
    rw [← even_iff_two_dvd] at hx; rw [← Int.odd_iff_not_even] at hx
  ...
  rw [Int.two_pow_sub_pow' d hxy4 _]; rw [sq_sub_sq]; rw [← Int.ofNat_mul_out]; rw [multiplicity.mul Int.prime_two]; rw [multiplicity.mul Int.prime_two]
...
theorem Nat.two_pow_sub_pow {x y : ℕ} (hxy : 2 ∣ x - y) (hx : ¬2 ∣ x) {n : ℕ} (hn : Even n) :
  ...
  · iterate 3 rw [← multiplicity.Int.coe_nat_multiplicity]
    ...
    rw [← Int.coe_nat_dvd] at hxy; rw [Int.ofNat_sub hyx] at hxy
    convert Int.two_pow_sub_pow hxy hx hn using 2
    rw [← multiplicity.Int.coe_nat_multiplicity]
    rfl
  · simp only [Nat.sub_eq_zero_iff_le.mpr hyx, Nat.sub_eq_zero_iff_le.mpr (pow_le_pow_of_le_left' hyx n), multiplicity.zero, PartENat.top_add, PartENat.add_top]
...
theorem pow_sub_pow (hyx : y < x) (hxy : p ∣ x - y) (hx : ¬p ∣ x) {n : ℕ} (hn : 0 < n) :
  ...
  rw [← PartENat.natCast_inj]; rw [Nat.cast_add]
