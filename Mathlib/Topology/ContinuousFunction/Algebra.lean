...
instance [CommGroup β] [TopologicalGroup β] : TopologicalGroup C(α, β) where
  continuous_mul := by
    ...
    rintro ⟨f, g⟩
    rw [ContinuousAt]; rw [tendsto_iff_forall_compact_tendstoUniformlyOn]; rw [nhds_prod_eq]
    exact fun K hK =>
  ...
  continuous_inv := by
    ...
    rw [continuous_iff_continuousAt]
    intro f
    rw [ContinuousAt]; rw [tendsto_iff_forall_compact_tendstoUniformlyOn]
    exact fun K hK =>
...
def compStarAlgEquiv' (f : X ≃ₜ Y) : C(Y, A) ≃⋆ₐ[𝕜] C(X, A) :=
  { f.toContinuousMap.compStarAlgHom' 𝕜 A with
    ...
    left_inv := fun g => by
      simp only [ContinuousMap.compStarAlgHom'_apply, ContinuousMap.comp_assoc, toContinuousMap_comp_symm, ContinuousMap.comp_id]
    right_inv := fun g => by
      simp only [ContinuousMap.compStarAlgHom'_apply, ContinuousMap.comp_assoc, symm_comp_toContinuousMap, ContinuousMap.comp_id]
