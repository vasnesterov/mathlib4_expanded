...
theorem continuous_pow (n : ℕ) : Continuous fun a : ℝ≥0∞ => a ^ n := by
  ...
  · simp only [H, pow_eq_top_iff, zero_ne_top, false_or_iff, eq_self_iff_true, not_true, Ne.def, not_false_iff, false_and_iff]
...
theorem iInf_mul_left' {ι} {f : ι → ℝ≥0∞} {a : ℝ≥0∞} (h : a = ⊤ → ⨅ i, f i = 0 → ∃ i, f i = 0)
  ...
  · rcases h H.1 H.2 with ⟨i, hi⟩
    rw [H.2]; rw [mul_zero]; rw [← bot_eq_zero]; rw [iInf_eq_bot]
...
theorem add_iSup {ι : Sort*} {s : ι → ℝ≥0∞} [Nonempty ι] : a + iSup s = ⨆ b, a + s b := by
  rw [add_comm]; rw [iSup_add]; simp [add_comm]
...
theorem finset_sum_iSup_nat {α} {ι} [SemilatticeSup ι] {s : Finset α} {f : α → ι → ℝ≥0∞}
  ...
  · intro a s has ih
    ...
    rw [ih]; rw [iSup_add_iSup_of_monotone (hf a)]
    intro i j h
...
theorem iSup_mul {ι : Sort*} {f : ι → ℝ≥0∞} {a : ℝ≥0∞} : iSup f * a = ⨆ i, f i * a := by
  rw [mul_comm]; rw [mul_iSup]; congr; funext; rw [mul_comm]
...
theorem exists_upcrossings_of_not_bounded_under {ι : Type*} {l : Filter ι} {x : ι → ℝ}
  ...
  rw [isBoundedUnder_le_abs] at hbdd; rw [not_and_or] at hbdd
  ...
  · obtain ⟨R, hR⟩ := exists_frequently_lt_of_liminf_ne_top hf
    ...
    · simp only [IsBoundedUnder, IsBounded, eventually_map, eventually_atTop, ge_iff_le, not_exists, not_forall, not_le, exists_prop] at hbdd
...
theorem tsum_add_one_eq_top {f : ℕ → ℝ≥0∞} (hf : ∑' n, f n = ∞) (hf0 : f 0 ≠ ∞) :
    ∑' n, f (n + 1) = ∞ := by
  rw [tsum_eq_zero_add' ENNReal.summable] at hf; rw [add_eq_top] at hf
...
theorem tendsto_iff_edist_tendsto_0 {l : Filter β} {f : β → α} {y : α} :
  ...
  simp only [EMetric.nhds_basis_eball.tendsto_right_iff, EMetric.mem_ball, @tendsto_order ℝ≥0∞ β _ _, forall_prop_of_false ENNReal.not_lt_zero, forall_const, true_and_iff]
