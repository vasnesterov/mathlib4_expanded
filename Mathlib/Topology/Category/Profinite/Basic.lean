  ...
  · -- Porting note: in mathlib3 `contrapose` saw through `Function.Surjective`.
    ...
    classical
      ...
      have H : h = g := by
        ...
        rw [comp_apply]; rw [ContinuousMap.coe_mk]; rw [comp_apply]; rw [ContinuousMap.coe_mk]; rw [Function.comp_apply]; rw [if_neg]
        refine' mt (fun α => hVU α) _
        simp only [Set.mem_range_self, not_true, not_false_iff, Set.mem_compl_iff]
      apply_fun fun e => (e y).down at H
      dsimp [LocallyConstant.ofClopen] at H
      rw [ContinuousMap.coe_mk] at H; rw [ContinuousMap.coe_mk] at H; rw [Function.comp_apply] at H; rw [if_pos hyV] at H
      exact top_ne_bot H
