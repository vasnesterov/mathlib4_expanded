...
theorem isSeparator_iff_faithful_preadditiveCoyoneda (G : C) :
  ...
  rw [isSeparator_iff_faithful_coyoneda_obj]; rw [← whiskering_preadditiveCoyoneda]; rw [Functor.comp_obj]; rw [whiskeringRight_obj_obj]
...
theorem isCoseparator_iff_faithful_preadditiveYoneda (G : C) :
  ...
  rw [isCoseparator_iff_faithful_yoneda_obj]; rw [← whiskering_preadditiveYoneda]; rw [Functor.comp_obj]; rw [whiskeringRight_obj_obj]
  exact ⟨fun h => Faithful.of_comp _ (forget AddCommGroupCat), fun h => Faithful.comp _ _⟩
