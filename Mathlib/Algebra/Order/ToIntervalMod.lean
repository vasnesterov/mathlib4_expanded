...
theorem toIocDiv_neg (a b : α) : toIocDiv hp a (-b) = -(toIcoDiv hp (-a) b + 1) := by
  rw [← neg_neg b]; rw [toIcoDiv_neg]; rw [neg_neg]; rw [neg_neg]; rw [neg_add']; rw [neg_neg]; rw [add_sub_cancel]
...
theorem tfae_modEq :
  ...
  tfae_have 4 → 3
  · intro h
    rw [← h]; rw [Ne]; rw [eq_comm]; rw [add_right_eq_self]
    exact hp.ne'
  tfae_have 1 → 4
  · intro h
...
theorem not_modEq_iff_toIcoDiv_eq_toIocDiv :
  ...
  rw [not_modEq_iff_toIcoMod_eq_toIocMod hp]; rw [toIcoMod]; rw [toIocMod]; rw [sub_right_inj]; rw [(zsmul_strictMono_left hp).injective.eq_iff]
...
theorem modEq_iff_toIcoDiv_eq_toIocDiv_add_one :
  ...
  rw [modEq_iff_toIcoMod_add_period_eq_toIocMod hp]; rw [toIcoMod]; rw [toIocMod]; rw [← eq_sub_iff_add_eq]; rw [sub_sub]; rw [sub_right_inj]; rw [← add_one_zsmul]; rw [(zsmul_strictMono_left hp).injective.eq_iff]
...
theorem toIocMod_le_toIcoMod_add (a b : α) : toIocMod hp a b ≤ toIcoMod hp a b + p := by
  rw [toIcoMod]; rw [toIocMod]; rw [sub_add]; rw [sub_le_sub_iff_left]; rw [sub_le_iff_le_add]; rw [← add_one_zsmul]; rw [(zsmul_strictMono_left hp).le_iff_le]
...
private theorem toIxxMod_cyclic_left {x₁ x₂ x₃ : α} (h : toIcoMod hp x₁ x₂ ≤ toIocMod hp x₁ x₃) :
  ...
  · obtain ⟨z, hd⟩ : ∃ z : ℤ, x₂ = x₂' + z • p := ((toIcoMod_eq_iff hp).1 rfl).2
    rw [hd]; rw [toIocMod_add_zsmul']; rw [toIcoMod_add_zsmul']; rw [add_le_add_iff_right]
...
private theorem toIxxMod_total' (a b c : α) :
  ...
  rw [_root_.add_comm (toIocMod _ _ _)] at this; rw [add_add_add_comm] at this; rw [← two_nsmul] at this
