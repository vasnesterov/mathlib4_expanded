...
theorem lieCharacter_apply_of_mem_derived (χ : LieCharacter R L) {x : L}
  ...
  rw [derivedSeries_def] at h; rw [derivedSeriesOfIdeal_succ] at h; rw [derivedSeriesOfIdeal_zero] at h; rw [←
    LieSubmodule.mem_coeSubmodule] at h; rw [LieSubmodule.lieIdeal_oper_eq_linear_span] at h
  refine' Submodule.span_induction h _ _ _ _
  · rintro y ⟨⟨z, hz⟩, ⟨⟨w, hw⟩, rfl⟩⟩; apply lieCharacter_apply_lie
  · exact χ.map_zero
  · intro y z hy hz; rw [LieHom.map_add]; rw [hy]; rw [hz]; rw [add_zero]
  · intro t y hy; rw [LieHom.map_smul]; rw [hy]; rw [smul_zero]
...
def lieCharacterEquivLinearDual [IsLieAbelian L] : LieCharacter R L ≃ Module.Dual R L where
  ...
  invFun ψ :=
    { ψ with
      map_lie' := fun {x y} => by
        rw [LieModule.IsTrivial.trivial]; rw [LieRing.of_associative_ring_bracket]; rw [mul_comm]; rw [sub_self]; rw [LinearMap.toFun_eq_coe]; rw [LinearMap.map_zero] }
