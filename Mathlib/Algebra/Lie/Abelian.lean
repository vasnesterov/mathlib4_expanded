...
theorem Function.Surjective.isLieAbelian {R : Type u} {L₁ : Type v} {L₂ : Type w} [CommRing R]
  ...
  { trivial := fun x y => by
      obtain ⟨u, rfl⟩ := h₁ x
      obtain ⟨v, rfl⟩ := h₁ y
      rw [← LieHom.map_lie]; rw [trivial_lie_zero]; rw [LieHom.map_zero] }
...
theorem ideal_oper_maxTrivSubmodule_eq_bot (I : LieIdeal R L) :
  ...
  rw [← LieSubmodule.coe_toSubmodule_eq_iff]; rw [LieSubmodule.lieIdeal_oper_eq_linear_span]; rw [LieSubmodule.bot_coeSubmodule]; rw [Submodule.span_eq_bot]
  ...
  toFun f :=
    { toLinearMap := f.val
      map_lie' := fun {x m} => by
        ...
        rw [LieHom.lie_apply] at hf; rw [sub_eq_zero] at hf; rw [← LinearMap.toFun_eq_coe] at hf; exact hf.symm}
...
theorem LieSubmodule.lie_abelian_iff_lie_self_eq_bot : IsLieAbelian I ↔ ⁅I, I⁆ = ⊥ := by
  simp only [_root_.eq_bot_iff, lieIdeal_oper_eq_span, LieSubmodule.lieSpan_le, LieSubmodule.bot_coe, Set.subset_singleton_iff, Set.mem_setOf_eq, exists_imp]
  refine'
    ⟨fun h z x y hz =>
      hz.symm.trans
