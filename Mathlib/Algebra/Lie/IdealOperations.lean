...
theorem lieIdeal_oper_eq_linear_span :
  ...
  · let s := { m : M | ∃ (x : ↥I) (n : ↥N), ⁅(x : L), (n : M)⁆ = m }
    have aux : ∀ (y : L), ∀ m' ∈ Submodule.span R s, ⁅y, m'⁆ ∈ Submodule.span R s := by
      ...
      · rintro m'' ⟨x, n, hm''⟩; rw [← hm'']; rw [leibniz_lie]
...
theorem mono_lie (h₁ : I ≤ J) (h₂ : N ≤ N') : ⁅I, N⁆ ≤ ⁅J, N'⁆ := by
  intro m h
  rw [lieIdeal_oper_eq_span] at h; rw [mem_lieSpan] at h; rw [lieIdeal_oper_eq_span]; rw [mem_lieSpan]
...
theorem lie_sup : ⁅I, N ⊔ N'⁆ = ⁅I, N⁆ ⊔ ⁅I, N'⁆ := by
  ...
  rw [lieIdeal_oper_eq_span]; rw [lieSpan_le]; rintro m ⟨x, ⟨n, hn⟩, h⟩; erw [LieSubmodule.mem_sup]
...
theorem sup_lie : ⁅I ⊔ J, N⁆ = ⁅I, N⁆ ⊔ ⁅J, N⁆ := by
  ...
  rw [lieIdeal_oper_eq_span]; rw [lieSpan_le]; rintro m ⟨⟨x, hx⟩, n, h⟩; erw [LieSubmodule.mem_sup]
...
theorem map_bracket_eq : map f ⁅I, N⁆ = ⁅I, map f N⁆ := by
  rw [← coe_toSubmodule_eq_iff]; rw [coeSubmodule_map]; rw [lieIdeal_oper_eq_linear_span]; rw [lieIdeal_oper_eq_linear_span]; rw [Submodule.map_span]
  congr
  ext m
  constructor
...
theorem map_bracket_eq {I₁ I₂ : LieIdeal R L} (h : Function.Surjective f) :
  ...
  rw [← LieSubmodule.coeSubmodule_le_coeSubmodule]; rw [coe_map_of_surjective h]; rw [LieSubmodule.lieIdeal_oper_eq_linear_span]; rw [LieSubmodule.lieIdeal_oper_eq_linear_span]; rw [LinearMap.map_span]
...
theorem comap_bracket_eq {J₁ J₂ : LieIdeal R L'} (h : f.IsIdealMorphism) :
  ...
  rw [← LieSubmodule.coe_toSubmodule_eq_iff]; rw [comap_coeSubmodule]; rw [LieSubmodule.sup_coe_toSubmodule]; rw [f.ker_coeSubmodule]; rw [← Submodule.comap_map_eq]; rw [LieSubmodule.lieIdeal_oper_eq_linear_span]; rw [LieSubmodule.lieIdeal_oper_eq_linear_span]; rw [LinearMap.map_span]
  ...
  · rintro ⟨x, ⟨⟨z₁, hz₁⟩, ⟨z₂, hz₂⟩, hx⟩, hy⟩; rw [← hy]; rw [← hx]
...
theorem comap_bracket_incl_of_le {I₁ I₂ : LieIdeal R L} (h₁ : I₁ ≤ I) (h₂ : I₂ ≤ I) :
    ...
    rw [comap_bracket_incl]; rw [← inf_eq_right] at h₁ h₂; rw [h₁]; rw [h₂]
