...
theorem div_dvd_of_dvd {p q : R} (hpq : q ∣ p) : p / q ∣ p := by
  ...
  · rw [hq, zero_dvd_iff] at hpq
    ...
    exact dvd_zero _
  use q
  rw [mul_comm]; rw [← EuclideanDomain.mul_div_assoc _ hpq]; rw [mul_comm]; rw [EuclideanDomain.mul_div_cancel _ hq]
...
theorem gcd_eq_gcd_ab (a b : R) : (gcd a b : R) = a * gcdA a b + b * gcdB a b := by
  have :=
    @xgcdAux_P _ _ _ a b a b 1 0 0 1 (by dsimp [P]; rw [mul_one]; rw [mul_zero]; rw [add_zero])
      (by dsimp [P]; rw [mul_one]; rw [mul_zero]; rw [zero_add])
...
theorem lcm_eq_zero_iff {x y : R} : lcm x y = 0 ↔ x = 0 ∨ y = 0 := by
  constructor
  · intro hxy
    rw [lcm] at hxy; rw [mul_div_assoc _ (gcd_dvd_right _ _)] at hxy; rw [mul_eq_zero] at hxy
