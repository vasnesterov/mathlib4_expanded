...
theorem smul_eq_map [MulSemiringAction M R] (m : M) :
  ...
  rw [Polynomial.map_monomial]; rw [Polynomial.smul_monomial]; rw [MulSemiringAction.toRingHom_apply]
...
theorem smul_eval_smul (m : M) (f : S[X]) (x : S) : (m • f).eval (m • x) = m • f.eval x :=
  Polynomial.induction_on f (fun r ↦ by rw [smul_C, eval_C, eval_C])
    ...
    rw [smul_mul']; rw [smul_pow']; rw [smul_C]; rw [smul_X]; rw [eval_mul]; rw [eval_C]; rw [eval_pow]; rw [eval_X]; rw [eval_mul]; rw [eval_C]; rw [eval_pow]; rw [eval_X]; rw [smul_mul']; rw [smul_pow']
...
protected noncomputable def polynomial (g : P →+*[M] Q) : P[X] →+*[M] Q[X] where
  ...
  map_smul' m p :=
    Polynomial.induction_on p
      ...
      fun n b _ ↦ by
      rw [smul_mul']; rw [smul_C]; rw [smul_pow']; rw [smul_X]; rw [Polynomial.map_mul]; rw [map_C]; rw [Polynomial.map_pow]; rw [map_X]; rw [coe_fn_coe]; rw [g.map_smul]; rw [Polynomial.map_mul]; rw [map_C]; rw [Polynomial.map_pow]; rw [map_X]; rw [smul_mul']; rw [smul_C]; rw [smul_pow']; rw [smul_X]; rw [coe_fn_coe]
