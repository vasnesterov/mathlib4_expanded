      ...
      have mul_right_inv : ∀ a, a * a⁻¹ = 1 := fun a =>
        calc a * a⁻¹ = 1 * (a * a⁻¹) := (one_mul _).symm
          ...
          _ = (a * a⁻¹)⁻¹ * (a * ((a⁻¹ * a) * a⁻¹)) := by
            simp only [assoc]
          _ = 1 := by
            rw [mul_left_inv]; rw [one_mul]; rw [mul_left_inv]
      rw [← mul_left_inv a]; rw [← assoc]; rw [mul_right_inv a]; rw [one_mul] }
  ...
  have mul_left_inv : ∀ a, a⁻¹ * a = 1 := fun a =>
    calc a⁻¹ * a = (a⁻¹ * a) * 1 := (mul_one _).symm
      ...
      _ = ((a⁻¹ * (a * a⁻¹)) * a) * (a⁻¹ * a)⁻¹ := by
        simp only [assoc]
      _ = 1 := by
        rw [mul_right_inv]; rw [mul_one]; rw [mul_right_inv]
  { mul_assoc := assoc,
    mul_one := mul_one,
    mul_left_inv := mul_left_inv,
    one_mul := fun a => by
      rw [← mul_right_inv a]; rw [assoc]; rw [mul_left_inv]; rw [mul_one] }
