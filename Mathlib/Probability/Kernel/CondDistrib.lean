...
theorem _root_.MeasureTheory.AEStronglyMeasurable.ae_integrable_condDistrib_map_iff
  ...
  rw [condDistrib]; rw [← hf.ae_integrable_condKernel_iff]; rw [Measure.fst_map_prod_mk₀ hY]
...
theorem _root_.MeasureTheory.AEStronglyMeasurable.integral_condDistrib_map
  ...
  rw [← Measure.fst_map_prod_mk₀ hY]; rw [condDistrib]; exact hf.integral_condKernel
...
theorem _root_.MeasureTheory.Integrable.condDistrib_ae_map
  ...
  rw [condDistrib]; rw [← Measure.fst_map_prod_mk₀ (X := X) hY]; exact hf_int.condKernel_ae
...
theorem _root_.MeasureTheory.Integrable.integral_norm_condDistrib_map
  ...
  rw [condDistrib]; rw [← Measure.fst_map_prod_mk₀ (X := X) hY]; exact hf_int.integral_norm_condKernel
...
theorem _root_.MeasureTheory.Integrable.norm_integral_condDistrib_map
    (hY : AEMeasurable Y μ) (hf_int : Integrable f (μ.map fun a => (X a, Y a))) :
    Integrable (fun x => ‖∫ y, f (x, y) ∂condDistrib Y X μ x‖) (μ.map X) := by
  rw [condDistrib]; rw [← Measure.fst_map_prod_mk₀ (X := X) hY]; exact hf_int.norm_integral_condKernel
...
theorem set_lintegral_preimage_condDistrib (hX : Measurable X) (hY : AEMeasurable Y μ)
  ...
  rw [lintegral_comp (kernel.measurable_coe _ hs) hX]; rw [condDistrib]; rw [← Measure.restrict_map hX ht]; rw [←
    Measure.fst_map_prod_mk₀ hY]; rw [set_lintegral_condKernel_eq_measure_prod _ ht hs]; rw [Measure.map_apply_of_aemeasurable (hX.aemeasurable.prod_mk hY) (ht.prod hs)]; rw [mk_preimage_prod]
...
theorem condDistrib_ae_eq_condexp (hX : Measurable X) (hY : Measurable Y) (hs : MeasurableSet s) :
  ...
  · intro t ht _
    rw [integral_toReal ((measurable_condDistrib hs).mono hX.comap_le le_rfl).aemeasurable
      (eventually_of_forall fun ω => measure_lt_top (condDistrib Y X μ (X ω)) _)]; rw [integral_indicator_const _ (hY hs)]; rw [Measure.restrict_apply (hY hs)]; rw [smul_eq_mul]; rw [mul_one]; rw [inter_comm]; rw [set_lintegral_condDistrib_of_measurableSet hX hY.aemeasurable hs ht]
...
theorem condexp_prod_ae_eq_integral_condDistrib' [NormedSpace ℝ F] [CompleteSpace F]
  ...
  · rintro s ⟨t, ht, rfl⟩ _
    ...
    rw [← Measure.restrict_map hX ht]; rw [← Measure.fst_map_prod_mk₀ hY]; rw [condDistrib]; rw [set_integral_condKernel_univ_right ht hf_int.integrableOn]; rw [set_integral_map (ht.prod MeasurableSet.univ) hf_int.1 (hX.aemeasurable.prod_mk hY)]; rw [mk_preimage_prod]; rw [preimage_univ]; rw [inter_univ]
...
theorem _root_.MeasureTheory.Integrable.comp_snd_map_prod_mk
  ...
  · have hf := hf_int.1.comp_snd_map_prod_mk X (mΩ := mΩ) (mβ := mβ)
    refine' ⟨hf, _⟩
    rw [HasFiniteIntegral]; rw [lintegral_map' hf.ennnorm (hX.prod_mk aemeasurable_id)]
    exact hf_int.2
  · rw [Measure.map_of_not_aemeasurable]
    · simp
    · contrapose! hX; exact measurable_fst.comp_aemeasurable hX
